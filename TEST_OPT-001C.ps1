...
$SummaryPath = Join-Path $Build "opt001c-test-summary.json"
$Steps = @()
$Failures = 0
$FirewallRules = @()
...
function Invoke-Live(
  ...
  Write-Host ("[MiniQuake/OPT-001C] starting {0}" -f $Name) -ForegroundColor Cyan
...
function Install-LoopbackFirewallRules {
  if ($SkipTemporaryFirewallRules) {
    Write-Host "[MiniQuake/OPT-001C] temporary loopback firewall rules explicitly skipped" -ForegroundColor Yellow
    return
  }
  if (-not (Test-IsAdministrator)) {
    throw "OPT-001C listen-server measurement requires elevated PowerShell for unattended temporary loopback firewall rules. Start PowerShell as Administrator or pass -SkipTemporaryFirewallRules."
  }
  ...
  Write-Host "[MiniQuake/OPT-001C] temporary loopback firewall rules installed: 2" -ForegroundColor Green
}
...
function Remove-LoopbackFirewallRules {
  ...
  if ($script:FirewallRules.Count -gt 0) {
    Write-Host "[MiniQuake/OPT-001C] temporary loopback firewall rules removed" -ForegroundColor Green
  }
}

function Write-Summary([string]$Overall, [string]$HandleClassification, [string]$PerformanceClassification) {
  [ordered]@{
    schema = "MiniQuakeOPT001CTestSummary/1"
...
Write-Host "MiniQuake OPT-001C acceptance test"
...
try {
  [void](Invoke-Live "OPT-001C package verification" $PythonExe @(
    (Join-Path $Root "tools\verify.py"), "--root", $Root,
    "--json", (Join-Path $Build "opt001c-package-verification.json")
  ) "opt001c-package-verification.log")
  ...
  if (-not $SkipBuild) {
    ...
    [void](Invoke-Live "OPT-001C game and contract build" $PowerShellExe $BuildArgs "opt001c-build.log")
  } else {
  ...
  foreach ($Map in @("e1m1", "e1m2")) {
    ...
    foreach ($Mode in @("headless", "render")) {
      ...
      ) ("opt001c-{0}-{1}-baseline.log" -f $Map, $Mode))
    }
  }

  $VisiblePrefix = Join-Path $Build "opt001c-e1m2-visible"
  [void](Invoke-Live "OPT-001C e1m2 visible validation" $GameExe @(
    "--render-evidence", $QuakeBase, "e1m2", [string]$E1M2VisibleFrames, $VisiblePrefix, "-game", $Game
  ) "opt001c-e1m2-visible.log")

  [void](Invoke-Live "OPT-001C e1m2 headless validation" $GameExe @(
    "--runtime-smoke", $QuakeBase, "e1m2", [string]$E1M2HeadlessFrames, "-game", $Game
  ) "opt001c-e1m2-headless.log")

  $TransitionPrefix = Join-Path $Build "opt001c-transition"
  [void](Invoke-Live "OPT-001C e1m1-e1m2-e1m1 transition" $GameExe @(
    "--opt001b-transition", $QuakeBase, [string]$TransitionFrames, $TransitionPrefix, "-game", $Game
  ) "opt001c-transition.log")

  $PlateauPrefix = Join-Path $Build "opt001c-listen-handle-plateau"
  ...
  ) "opt001c-handle-plateau.log" -AllowFailure)

  [void](Invoke-Live "OPT-001C aggregate analysis" $PythonExe @(
    (Join-Path $Root "tools\analyze_opt001a.py"), "--build", $Build,
    "--json", (Join-Path $Build "opt001c-baseline-summary.json"),
    "--markdown", (Join-Path $Build "opt001c-baseline-summary.md")
  ) "opt001c-aggregate-analysis.log" -AllowFailure)

  $PerformanceJson = Join-Path $Build "opt001c-performance-comparison.json"
  [void](Invoke-Live "OPT-001C performance comparison" $PythonExe @(
    ...
    "--markdown", (Join-Path $Build "opt001c-performance-comparison.md")
  ) "opt001c-performance-comparison.log")
  ...
  Write-Host ""
  Write-Host "MiniQuake OPT-001C summary"
  ...
  if ($Overall -eq "PASS") {
    Write-Host "MiniQuake OPT-001C acceptance test: PASS" -ForegroundColor Green
    exit 0
  }
  Write-Host ("MiniQuake OPT-001C acceptance test: {0}" -f $Overall) -ForegroundColor Red
  exit 2
}
catch {
  Add-Step "OPT-001C harness" "FAIL" 1 $_.Exception.Message
  Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  Write-Host ("ERROR: " + $_.Exception.Message) -ForegroundColor Red
  Write-Host "MiniQuake OPT-001C acceptance test: FAIL" -ForegroundColor Red
  exit 1
}
finally {
