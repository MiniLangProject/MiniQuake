...
$SummaryPath = Join-Path $Build "opt001cr2-test-summary.json"
...
function Invoke-LivePowerShellScript(
  ...
  Write-Host ("[MiniQuake/OPT-001CR2] starting {0}" -f $Name) -ForegroundColor Cyan
...
function Install-LoopbackFirewallRules {
  if ($SkipTemporaryFirewallRules) {
    Write-Host "[MiniQuake/OPT-001CR2] temporary loopback firewall rules explicitly skipped" -ForegroundColor Yellow
    return
  }
  if (-not (Test-IsAdministrator)) {
    throw "OPT-001CR2 listen-server measurement requires elevated PowerShell for unattended temporary loopback firewall rules. Start PowerShell as Administrator or pass -SkipTemporaryFirewallRules."
  }
  ...
  Write-Host "[MiniQuake/OPT-001CR2] temporary loopback firewall rules installed: 2" -ForegroundColor Green
}
...
function Remove-LoopbackFirewallRules {
  ...
  if ($script:FirewallRules.Count -gt 0) {
    Write-Host "[MiniQuake/OPT-001CR2] temporary loopback firewall rules removed" -ForegroundColor Green
  }
}
...
Write-Host "MiniQuake OPT-001CR2 acceptance test"
...
try {
  [void](Invoke-Live "OPT-001CR2 package verification" $PythonExe @(
    (Join-Path $Root "tools\verify.py"), "--root", $Root,
    "--json", (Join-Path $Build "opt001cr2-package-verification.json")
  ) "opt001cr2-package-verification.log")
  ...
  if (-not $SkipBuild) {
    ...
    [void](Invoke-LivePowerShellScript `
      "OPT-001CR2 game and contract build" `
  ...
  foreach ($Map in @("e1m1", "e1m2")) {
    ...
    foreach ($Mode in @("headless", "render")) {
      ...
      ) ("opt001cr2-{0}-{1}-baseline.log" -f $Map, $Mode))
    }
  }

  $VisiblePrefix = Join-Path $Build "opt001cr2-e1m2-visible"
  [void](Invoke-Live "OPT-001CR2 e1m2 visible validation" $GameExe @(
    "--render-evidence", $QuakeBase, "e1m2", [string]$E1M2VisibleFrames, $VisiblePrefix, "-game", $Game
  ) "opt001cr2-e1m2-visible.log")

  [void](Invoke-Live "OPT-001CR2 e1m2 headless validation" $GameExe @(
    "--runtime-smoke", $QuakeBase, "e1m2", [string]$E1M2HeadlessFrames, "-game", $Game
  ) "opt001cr2-e1m2-headless.log")

  $TransitionPrefix = Join-Path $Build "opt001cr2-transition"
  [void](Invoke-Live "OPT-001CR2 e1m1-e1m2-e1m1 transition" $GameExe @(
    "--opt001b-transition", $QuakeBase, [string]$TransitionFrames, $TransitionPrefix, "-game", $Game
  ) "opt001cr2-transition.log")

  $PlateauPrefix = Join-Path $Build "opt001cr2-listen-handle-plateau"
  ...
  ) "opt001cr2-handle-plateau.log" -AllowFailure)

  [void](Invoke-Live "OPT-001CR2 aggregate analysis" $PythonExe @(
    (Join-Path $Root "tools\analyze_opt001a.py"), "--build", $Build,
    "--prefix", "opt001cr2", "--next-revision", "OPT-001D",
    "--json", (Join-Path $Build "opt001cr2-baseline-summary.json"),
    "--markdown", (Join-Path $Build "opt001cr2-baseline-summary.md")
  ) "opt001cr2-aggregate-analysis.log" -AllowFailure)

  $PerformanceJson = Join-Path $Build "opt001cr2-performance-comparison.json"
  [void](Invoke-Live "OPT-001CR2 performance comparison" $PythonExe @(
    ...
    "--markdown", (Join-Path $Build "opt001cr2-performance-comparison.md")
  ) "opt001cr2-performance-comparison.log")
  ...
  Write-Host ""
  Write-Host "MiniQuake OPT-001CR2 summary"
  ...
  if ($Overall -eq "PASS") {
    Write-Host "MiniQuake OPT-001CR2 acceptance test: PASS" -ForegroundColor Green
    exit 0
  }
  Write-Host ("MiniQuake OPT-001CR2 acceptance test: {0}" -f $Overall) -ForegroundColor Red
  exit 2
}
catch {
  Add-Step "OPT-001CR2 harness" "FAIL" 1 $_.Exception.Message
  Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  Write-Host ("ERROR: " + $_.Exception.Message) -ForegroundColor Red
  Write-Host "MiniQuake OPT-001CR2 acceptance test: FAIL" -ForegroundColor Red
  exit 1
}
finally {
