#!/usr/bin/env python3
"""Direct pinned-source differential for WinQuake/gl_screen.c."""
...
PATCH = ROOT / "reference" / "patches" / "gl_screen_pinned_oracle.patch"
STUB_INCLUDE = ROOT / "reference" / "harness"
DRIVER = ROOT / "reference" / "harness" / "gl_screen_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "gl_screen_oracle.def"
FIXTURE = ROOT / "tests" / "gl_screen_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "gl_screen_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "gl_screen_pinned.obj"
    driver_obj = output / "gl_screen_pinned_driver.obj"
    dll = output / "gl_screen_oracle.dll"
    flags = [
        compiler, "/nologo", "/c", "/W4", "/wd4100", "/wd4101",
        "/wd4244", "/wd4706", "/GS-", "/Zl", "/fp:precise", "/O2",
        "/Gy", f"/I{STUB_INCLUDE}",
    ]
    base.run(
        flags
        + [
            ...
            str(worktree / "WinQuake" / "gl_screen.c"),
        ]
    )
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.gl_screen_oracle_jsonl
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "gl_screen_differential",
    )
    ...
    base.verify_reference()
    print("gl_screen differential: PASS (23 JSONL events, epsilon=1e-5)")
    return 0
