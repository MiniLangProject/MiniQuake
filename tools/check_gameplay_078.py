...
def main():
    ...
    for marker in ['MiniQuake BP-078 statusbar/scoreboard tests passed: 22','function testSortStableTie()','function testFaceHealthBands()']:
    ...
    expected={'fixtures':22,'statusbar_height':24,'statusbar_width':320,'max_scoreboard':16,'palette_offset':8,'health_faces':5,'normal_weapons':7}
    for k,v in expected.items():
        if golden.get(k)!=v: errors.append(f'golden {k}: expected {v}, got {golden.get(k)!r}')
    report={'schema_version':1,'package':'BP-078','status':'PASS' if not errors else 'FAIL','errors':errors,'fixtures':22}
