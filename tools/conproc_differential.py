#!/usr/bin/env python3
"""Direct pinned-source differential for WinQuake/conproc.c."""
...
PATCH = ROOT / "reference" / "patches" / "conproc_pinned_oracle.patch"
STUB_INCLUDE = ROOT / "reference" / "harness"
DRIVER = ROOT / "reference" / "harness" / "conproc_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "conproc_oracle.def"
FIXTURE = ROOT / "tests" / "conproc_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "conproc_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "conproc_pinned.obj"
    driver_obj = output / "conproc_pinned_driver.obj"
    dll = output / "conproc_oracle.dll"
    base = [
        compiler,
        "/nologo",
        "/c",
    ...
    run(
        ...
        + [
            ...
            f"/Fo{source_obj}",
            str(worktree / "WinQuake" / "conproc.c"),
        ]
    )
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.conproc_oracle_jsonl
...
def build_and_run_minilang(
    ...
    executable = output / "conproc_minilang.exe"
    shutil.copy2(
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "conproc_differential",
    )
    ...
    verify_reference()
    print("conproc differential: PASS (11 JSONL events, epsilon=0)")
    return 0
