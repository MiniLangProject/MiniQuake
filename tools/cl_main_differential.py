#!/usr/bin/env python3
"""Build pinned cl_main.c with stubs and compare all active MiniLang bodies."""
...
PINNED_SOURCE = ROOT / "reference" / "quake" / "WinQuake" / "cl_main.c"
HARNESS = ROOT / "reference" / "harness" / "cl_main_oracle.c"
DEFINITION = ROOT / "reference" / "harness" / "cl_main_oracle.def"
FIXTURE = ROOT / "tests" / "cl_main_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "cl_main_differential_manifest.json"
VERIFY_REFERENCE = ROOT / "tools" / "verify_reference.py"
MARKER = "/*__PINNED_CL_MAIN_SOURCE__*/"
...
def build_oracle(output: Path) -> Path:
    ...
    obj = output / "cl_main_oracle.obj"
    dll = output / "cl_main_oracle.dll"
    run(
        [
            compiler,
            "/nologo",
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.cl_main_oracle_jsonl
...
def build_and_run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "cl_main_minilang.exe"
...
def validate_manifest(reference: Path) -> None:
    ...
    if manifest["reference_execution"] != "direct-pinned-source-build":
        raise RuntimeError("cl_main source execution classification is not strict")
    ...
    if {item["classification"] for item in manifest["functions"]} - allowed:
        raise RuntimeError("cl_main manifest contains a non-strict classification")
    events = [
        json.loads(line)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "cl_main_differential",
    )
    ...
    print(f"cl_main differential: PASS ({events} JSONL events, epsilon=1e-5)")
