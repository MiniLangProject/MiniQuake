#!/usr/bin/env python3
"""Pinned WinQuake cl_input.c differential."""
...
SOURCE = ROOT / "reference/quake/WinQuake/cl_input.c"
STUBS = ROOT / "reference/harness/cl_input_oracle_stubs.h"
DRIVER = ROOT / "reference/harness/cl_input_pinned_driver.c"
DEFINITION = ROOT / "reference/harness/cl_input_oracle.def"
FIXTURE = ROOT / "tests/cl_input_differential_fixture.ml"
MANIFEST = ROOT / "audit/cl_input_differential_manifest.json"
...
def build(output):
    ...
    if source_text.count(include) != 1:
        raise RuntimeError("pinned cl_input.c include boundary changed")
    generated = output / "cl_input_oracle_generated.c"
    generated.write_text(
        source_text.replace(include, '#include "cl_input_oracle_stubs.h"'),
        encoding="utf-8",
    )
    common = [
        compiler,
    ...
    source_obj = output / "cl_input.obj"
    driver_obj = output / "cl_input_driver.obj"
    run(common + [f"/Fo{source_obj}", str(generated)])
    run(common + [f"/Fo{driver_obj}", str(DRIVER)])
    dll = output / "cl_input_oracle.dll"
    run([
        linker,
        "/dll",
        "/noentry",
...
def oracle(dll, destination):
    ...
    function = library.cl_input_oracle_jsonl
...
def candidate(compiler, output, destination):
    executable = output / "cl_input_minilang.exe"
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build/cl_input_differential",
    )
    ...
    if manifest["reference_execution"] != "direct-pinned-source-build":
        raise RuntimeError("cl_input source classification is not strict")
    ...
    run([sys.executable, str(VERIFY_REFERENCE)])
    print(f"cl_input differential: PASS ({len(events)} events, epsilon=1e-5)")
    return 0
