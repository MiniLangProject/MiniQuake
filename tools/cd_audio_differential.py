#!/usr/bin/env python3
"""Direct pinned-source differential for WinQuake/cd_win.c."""
...
PATCH = ROOT / "reference" / "patches" / "cd_audio_pinned_oracle.patch"
STUB_INCLUDE = ROOT / "reference" / "harness"
DRIVER = ROOT / "reference" / "harness" / "cd_audio_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "cd_audio_oracle.def"
FIXTURE = ROOT / "tests" / "cd_audio_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "cd_audio_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "cd_audio_pinned.obj"
    driver_obj = output / "cd_audio_pinned_driver.obj"
    dll = output / "cd_audio_oracle.dll"
    base = [
        compiler,
        "/nologo",
        "/c",
    ...
    run(
        ...
        + [
            ...
            f"/Fo{source_obj}",
            str(worktree / "WinQuake" / "cd_win.c"),
        ]
    )
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.cd_audio_oracle_jsonl
...
def build_and_run_minilang(
    ...
    executable = output / "cd_audio_minilang.exe"
    shutil.copy2(
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "cd_audio_differential",
    )
    ...
    verify_reference()
    print("cd_audio differential: PASS (8 JSONL events, epsilon=1e-5)")
    return 0
