#!/usr/bin/env python3
"""Direct pinned-source differential for WinQuake/console.c."""
...
PATCH = ROOT / "reference" / "patches" / "console_pinned_oracle.patch"
STUB_INCLUDE = ROOT / "reference" / "harness"
DRIVER = ROOT / "reference" / "harness" / "console_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "console_oracle.def"
FIXTURE = ROOT / "tests" / "console_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "console_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    compiler_text, linker_text, _ = tools
    ...
    source_obj = output / "console_pinned.obj"
    driver_obj = output / "console_pinned_driver.obj"
    dll = output / "console_oracle.dll"
    base = [
        compiler,
        "/nologo",
        "/c",
    ...
    run(
        ...
        + [
            ...
            f"/Fo{source_obj}",
            str(worktree / "WinQuake" / "console.c"),
        ]
    )
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.console_oracle_jsonl
...
def build_and_run_minilang(
    ...
    executable = output / "console_minilang.exe"
    game = output / "game"
    ...
    raw = run([str(executable), str(output)], capture=True)
    json_lines = [
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "console_differential",
    )
    ...
    verify_reference()
    print("console differential: PASS (17 JSONL events, epsilon=1e-5)")
    return 0
