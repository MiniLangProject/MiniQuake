#!/usr/bin/env python3
"""Build pinned host_cmd.c with stubs and compare all active GLQuake bodies."""
...
PINNED_SOURCE = ROOT / "reference" / "quake" / "WinQuake" / "host_cmd.c"
HARNESS = ROOT / "reference" / "harness" / "host_cmd_oracle.c"
DEFINITION = ROOT / "reference" / "harness" / "host_cmd_oracle.def"
FIXTURE = ROOT / "tests" / "host_cmd_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "host_cmd_differential_manifest.json"
VERIFY_REFERENCE = ROOT / "tools" / "verify_reference.py"
MARKER = "/*__PINNED_HOST_CMD_SOURCE__*/"
...
def materialize_pinned_source(output: Path) -> Path:
    ...
    if harness.count(MARKER) != 1:
        raise RuntimeError("host_cmd oracle harness marker is missing or ambiguous")
    source = PINNED_SOURCE.read_text(encoding="utf-8")
    include = '#include "quakedef.h"'
    if source.count(include) != 1:
        raise RuntimeError("pinned host_cmd.c include boundary changed")
    source = source.replace(include, "/* quakedef.h replaced by deterministic harness stubs */")
    generated = output / "host_cmd_oracle_generated.c"
...
def build_oracle(output: Path) -> Path:
    ...
    obj = output / "host_cmd_oracle.obj"
    dll = output / "host_cmd_oracle.dll"
    run(
        [
            compiler,
            "/nologo",
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.host_cmd_oracle_jsonl
...
def build_and_run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "host_cmd_minilang.exe"
...
def validate_manifest(reference: Path) -> None:
    ...
    if manifest["reference_execution"] != "direct-pinned-source-build":
        raise RuntimeError("host_cmd source execution classification is not strict")
    ...
    if {item["classification"] for item in manifest["functions"]} - allowed:
        raise RuntimeError("host_cmd manifest contains a non-strict classification")
    events = [
        json.loads(line)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "host_cmd_differential",
    )
    ...
    print(f"host_cmd differential: PASS ({events} JSONL events, epsilon=1e-5)")
