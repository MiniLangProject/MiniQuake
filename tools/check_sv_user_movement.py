...
"""Verify the check sv user movement compatibility and regression contract."""
...
def doc(root):
    ...
    return {'schema':'MiniQuakeSvUserMovementGolden/1','package_id':PACKAGE,'parent_package_id':PARENT,'sources':['sv_user.c','sv_phys.c'],'rows':rows(),'reference':{'oracle':ORACLE,'oracle_sha256':sha(root/ORACLE)}}
def cc():
...
def same(a,b):
 """Compare same and report the first mismatch."""
