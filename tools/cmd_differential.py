#!/usr/bin/env python3
"""Build pinned cmd.c with stubs and compare all active MiniLang bodies."""
...
PINNED_SOURCE = ROOT / "reference" / "quake" / "WinQuake" / "cmd.c"
HARNESS = ROOT / "reference" / "harness" / "cmd_oracle.c"
DEFINITION = ROOT / "reference" / "harness" / "cmd_oracle.def"
FIXTURE = ROOT / "tests" / "cmd_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "cmd_differential_manifest.json"
VERIFY_REFERENCE = ROOT / "tools" / "verify_reference.py"
MARKER = "/*__PINNED_CMD_SOURCE__*/"
...
def build_oracle(output: Path) -> Path:
    ...
    obj = output / "cmd_oracle.obj"
    dll = output / "cmd_oracle.dll"
    run([
        compiler, "/nologo", "/c", "/W4", "/GS-", "/Zl", "/fp:precise", "/O2",
        f"/Fo{obj}", str(generated),
    ])
    run([
        linker, "/dll", "/noentry", "/machine:x64", "/nodefaultlib",
        "/dynamicbase", "/nxcompat", f"/def:{DEFINITION}", f"/out:{dll}",
        str(obj), str(native_build / "msvcrt.lib"), str(native_build / "kernel32.lib"),
    ])
    return dll
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.cmd_oracle_jsonl
...
def build_and_run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "cmd_minilang.exe"
    ...
    run([
        ...
        "-I", str(ROOT / "src"), "-I", str(compiler.parent),
        "--keep-going", "--max-errors", "50",
        "--heap-reserve", "512m", "--heap-commit", "32m", "--heap-grow", "4m",
    ])
...
def validate_manifest(reference: Path) -> None:
    ...
    if manifest["reference_execution"] != "direct-pinned-source-build":
        raise RuntimeError("cmd source execution classification is not strict")
    allowed = {"reference-differential", "reference-artifact-byte-exact"}
    if {item["classification"] for item in manifest["functions"]} - allowed:
        raise RuntimeError("cmd manifest contains a non-strict classification")
    events = [json.loads(line) for line in reference.read_text(encoding="utf-8").splitlines() if line.strip()]
    observed = {event["function"] for event in events}
    expected = {item["name"] for item in manifest["functions"]}
    if expected - observed:
        raise RuntimeError(f"differential functions lack events: {sorted(expected - observed)}")
    if len(events) != manifest["events"]:
        raise RuntimeError(f"manifest expects {manifest['events']} events, observed {len(events)}")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--compiler", type=Path, default=ROOT.parent / "MiniLangCompilerPy" / "mlc_win64.py")
    parser.add_argument("--output", type=Path, default=ROOT / "build" / "cmd_differential")
    ...
    run([
        sys.executable, str(ROOT / "tools" / "parity_oracle.py"), "compare-traces",
        str(reference), str(candidate), "--epsilon", "1e-5",
    ])
    run([sys.executable, str(VERIFY_REFERENCE)])
    events = sum(1 for line in reference.read_text(encoding="utf-8").splitlines() if line)
    print(f"cmd differential: PASS ({events} JSONL events, epsilon=1e-5)")
