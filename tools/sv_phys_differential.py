#!/usr/bin/env python3
"""Build/compare the pinned GLQuake and MiniLang sv_phys fixtures."""
...
DRIVER = ROOT / "reference" / "harness" / "sv_phys_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "sv_phys_oracle.def"
STUB_INCLUDE = ROOT / "reference" / "harness"
PATCH = ROOT / "reference" / "patches" / "sv_phys_pinned_oracle.patch"
FIXTURE = ROOT / "tests" / "sv_phys_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "sv_phys_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "sv_phys_pinned.obj"
    driver_obj = output / "sv_phys_pinned_driver.obj"
    common = [
    ...
    run(common + [
        ...
        str(worktree / "WinQuake" / "sv_phys.c"),
    ])
    run(common + [f"/Fo{driver_obj}", str(DRIVER)])
    dll = output / "sv_phys_oracle.dll"
    run([
...
def run_oracle(dll: Path, destination: Path) -> None:
    ...
    function = library.sv_phys_oracle_jsonl
...
def run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "sv_phys_minilang.exe"
    shutil.copy2(
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=ROOT / "build" / "sv_phys_differential",
    )
    ...
    run_minilang(args.compiler.resolve(), args.output, candidate)
    validate(reference)
    run([
    ...
    print(f"sv_phys differential: PASS ({count} events, epsilon=1e-5)")
    return 0
