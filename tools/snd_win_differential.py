#!/usr/bin/env python3
"""Direct pinned-source differential for WinQuake/snd_win.c."""
...
PATCH = ROOT / "reference" / "patches" / "snd_win_pinned_oracle.patch"
STUB_INCLUDE = ROOT / "reference" / "harness"
DRIVER = ROOT / "reference" / "harness" / "snd_win_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "snd_win_oracle.def"
FIXTURE = ROOT / "tests" / "snd_win_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "snd_win_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "snd_win_pinned.obj"
    driver_obj = output / "snd_win_pinned_driver.obj"
    dll = output / "snd_win_oracle.dll"
    base = [
        compiler,
        "/nologo",
        "/c",
    ...
    run(
        ...
        + [
            ...
            f"/Fo{source_obj}",
            str(worktree / "WinQuake" / "snd_win.c"),
        ]
    )
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.snd_win_oracle_jsonl
...
def build_and_run_minilang(
    ...
    executable = output / "snd_win_minilang.exe"
    shutil.copy2(
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "snd_win_differential",
    )
    ...
    verify_reference()
    print("snd_win differential: PASS (9 JSONL events, epsilon=0)")
    return 0
