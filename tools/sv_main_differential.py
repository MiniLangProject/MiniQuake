#!/usr/bin/env python3
"""Build/compare the pinned WinQuake sv_main.c and MiniLang fixtures."""
...
DRIVER = ROOT / "reference" / "harness" / "sv_main_pinned_driver.c"
DEFINITION = ROOT / "reference" / "harness" / "sv_main_oracle.def"
STUB_INCLUDE = ROOT / "reference" / "harness"
PATCH = ROOT / "reference" / "patches" / "sv_main_pinned_oracle.patch"
FIXTURE = ROOT / "tests" / "sv_main_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "sv_main_differential_manifest.json"
...
def build_oracle(output: Path) -> Path:
    ...
    source_obj = output / "sv_main_pinned.obj"
    driver_obj = output / "sv_main_pinned_driver.obj"
    common = [
    ...
    run(common + [
        ...
        str(worktree / "WinQuake" / "sv_main.c"),
    ])
    run(common + [f"/Fo{driver_obj}", str(DRIVER)])
    dll = output / "sv_main_oracle.dll"
    run([
...
def run_oracle(dll: Path, destination: Path) -> None:
    ...
    function = library.sv_main_oracle_jsonl
...
def run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "sv_main_minilang.exe"
    shutil.copy2(
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=ROOT / "build" / "sv_main_differential",
    )
    ...
    print(f"sv_main differential: PASS ({count} events, epsilon=1e-5)")
    return 0
