#!/usr/bin/env python3
"""Build pinned host.c with stubs and compare all active GLQuake bodies."""
...
PINNED_SOURCE = ROOT / "reference" / "quake" / "WinQuake" / "host.c"
HARNESS = ROOT / "reference" / "harness" / "host_oracle.c"
DEFINITION = ROOT / "reference" / "harness" / "host_oracle.def"
FIXTURE = ROOT / "tests" / "host_differential_fixture.ml"
MANIFEST = ROOT / "audit" / "host_differential_manifest.json"
VERIFY_REFERENCE = ROOT / "tools" / "verify_reference.py"
MARKER = "/*__PINNED_HOST_SOURCE__*/"
...
def materialize_pinned_source(output: Path) -> Path:
    ...
    if harness.count(MARKER) != 1:
        raise RuntimeError("host oracle harness marker is missing or ambiguous")
    source = PINNED_SOURCE.read_text(encoding="utf-8")
    includes = ['#include "quakedef.h"', '#include "r_local.h"']
    for include in includes:
        if source.count(include) != 1:
            raise RuntimeError(f"pinned host.c include boundary changed: {include}")
        source = source.replace(include, "/* include replaced by deterministic harness stubs */")
    generated = output / "host_oracle_generated.c"
...
def build_oracle(output: Path) -> Path:
    ...
    obj = output / "host_oracle.obj"
    dll = output / "host_oracle.dll"
    run(
        [
            compiler,
            "/nologo",
    ...
    run(
        [
            ...
            f"/out:{dll}",
            str(obj),
            str(native_build / "msvcrt.lib"),
            str(native_build / "kernel32.lib"),
        ]
    )
    return dll
...
def run_oracle(dll_path: Path, destination: Path) -> None:
    ...
    function = library.host_oracle_jsonl
...
def build_and_run_minilang(compiler: Path, output: Path, destination: Path) -> None:
    executable = output / "host_minilang.exe"
...
def validate_manifest(reference: Path) -> None:
    ...
    if manifest["reference_execution"] != "direct-pinned-source-build":
        raise RuntimeError("host source execution classification is not strict")
    ...
    if {item["classification"] for item in manifest["functions"]} - allowed:
        raise RuntimeError("host manifest contains a non-strict classification")
    events = [
        json.loads(line)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build" / "host_differential",
    )
    ...
    print(f"host differential: PASS ({events} JSONL events, epsilon=1e-5)")
