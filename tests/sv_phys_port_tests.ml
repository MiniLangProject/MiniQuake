...
function main(args)
  ...
  if result is error then print "velocity/gravity/think failed"; return 1 end if
  print "[2/5] clip and pusher rollback order"
  result = try(testClipAndPusherRollbackOrder())
  if result is error then print "clip/pusher failed"; return 1 end if
  ...
  print "SV_PHYS PORT TESTS PASSED (5/5)"
  return 0
end function
