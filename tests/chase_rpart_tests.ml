...
function testDrawPhysicsAndTexture()
  ...
  softTexture = particleRender.softParticleTexturePixels()
  assertEqual(len(softTexture), 1024, "enhanced particle texture byte count")
  assertEqual(softTexture[3], 0, "enhanced particle transparent corner")
  assertTrue(softTexture[(8 * 16 + 8) * 4 + 3] > softTexture[3], "enhanced particle feathered center")
  assertEqual(particleRender.ConfigureEnhancedParticles(true), true, "enhanced particles enabled")
  assertEqual(particleRender.ConfigureEnhancedParticles(false), false, "classic particles restored")
