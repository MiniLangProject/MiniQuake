...
function testDescriptionsAndMenu()
  ...
  assertEqual(trace[5][0], "texture_upscale", "video menu texture-upscale entry")
  assertEqual(trace[6][0], "renderer", "video menu renderer entry")
  assertEqual(trace[7][0], "display", "video menu display entry")
  assertEqual(trace[10][3], true, "video menu current mode highlight")
  ...
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "renderer entry reaches texture scaling")
  ...
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "texture scaling reaches shadow quality")
