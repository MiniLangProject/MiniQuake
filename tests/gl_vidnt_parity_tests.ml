...
function testDescriptionsAndMenu()
  ...
  state.registry.variables = [
    ...
    cvar.create("r_modelinterpolate", "1", true, false),
    cvar.create("r_textureupscale", "0", true, false),
    cvar.create("r_anisotropy", "4", true, false),
  ]
  ...
  assertEqual(trace[5][0], "model_interpolation", "video menu model-interpolation entry")
  assertEqual(trace[6][0], "texture_upscale", "video menu texture-upscale entry")
  assertEqual(trace[7][0], "anisotropy", "video menu anisotropy entry")
  assertEqual(trace[8][0], "renderer", "video menu renderer entry")
  assertEqual(trace[9][0], "display", "video menu display entry")
  assertEqual(trace[12][3], true, "video menu current mode highlight")
  ...
  assertEqual(video.VID_MenuTextureUpscaleFocused(), false, "texture-upscale entry is not initially focused")
  assertEqual(video.VID_MenuAnisotropyFocused(), false, "anisotropy entry is not initially focused")
  assertEqual(video.VID_MenuModelInterpolationFocused(), false, "model-interpolation entry is not initially focused")
  ...
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "renderer entry reaches anisotropy")
  assertEqual(video.VID_MenuAnisotropyFocused(), true, "anisotropy entry has explicit focus")
  assertEqual(video.VID_MenuKey(keys.K_RIGHTARROW), "lighting_applied", "anisotropy cycles")
  assertEqual(cvar.variableValue(state.registry, "r_anisotropy"), 8.0, "anisotropy setting archived")
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "anisotropy reaches texture scaling")
  assertEqual(video.VID_MenuTextureUpscaleFocused(), true, "texture-upscale entry has explicit focus")
  upscaleAction = video.VID_MenuKey(keys.K_RIGHTARROW)
  assertEqual(upscaleAction[0], "texture_upscale", "texture upscaling requests resource reload")
  assertEqual(upscaleAction[1], 0, "texture upscaling reports previous mode")
  assertEqual(upscaleAction[2], 1, "texture upscaling reports selected mode")
  assertEqual(upscaleAction[3], "NEAREST 2X", "texture upscaling reports selected name")
  assertEqual(cvar.variableValue(state.registry, "r_textureupscale"), 1.0, "texture upscaling setting archived")
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "texture scaling reaches model interpolation")
  assertEqual(video.VID_MenuModelInterpolationFocused(), true, "model interpolation has explicit focus")
  assertEqual(video.VID_MenuKey(keys.K_LEFTARROW), "lighting_applied", "model interpolation toggles")
  assertEqual(cvar.variableValue(state.registry, "r_modelinterpolate"), 0.0, "model interpolation setting archived")
  assertEqual(video.VID_MenuKey(keys.K_UPARROW), "move", "model interpolation reaches shadow quality")
