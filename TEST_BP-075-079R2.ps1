...
$PackageId = "BP-079"
$ParentPackageId = "BP-078"
$BlockId = "BP-075-079"
$DeliveryRevision = "BP-075-079R2"
$DeliveryParent = "BP-075-079R1"
...
$TranscriptPath = Join-Path $Build ("bp075-079r2-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $Build "bp075-079r2-test-summary.json"
...
function Run-NetworkEvidencePair([string]$Suffix, [int]$Port) {
  $ServerOut = Join-Path $Build ("bp075-079r2-network-server-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp075-079r2-network-server-{0}.err.log" -f $Suffix)
  $ClientLog = Join-Path $Build ("bp075-079r2-network-client-{0}.log" -f $Suffix)
  $PairJson = Join-Path $Build ("bp075-079r2-network-pair-{0}.json" -f $Suffix)
...
$Failure = ""
try {
  Write-Host "MiniQuake BP-075-079R2 acceptance test"
  ...
  if (-not $SkipBuild) {
    ...
    [void](Run-Logged "single cumulative build and unit-test suite" $PowerShellExe $Arguments "bp075-079r2-build-child.log" "MiniQuake BP-079 gameplay/presentation closure tests passed: 24")
  } else {
  ...
  $Version = Run-Logged "package identity" $GameExe @("--version") "bp075-079r2-version.txt" "Gameplay/presentation status: gameplay_presentation_109_frozen_v1"
  if ($Version -notmatch 'Package: BP-079' -or $Version -notmatch 'Block: BP-075-079') { throw "compiled package identity mismatch" }
  ...
  if ($CanUseGame) {
    ...
    [void](Run-Logged "installed Quake data validation" $GameExe @("--validate-game", $QuakeBase, $Map, "-game", $Game) "bp075-079r2-game-validation.log" "Validation result: PASS")
    [void](Run-Logged "headless runtime validation" $GameExe @("--validate-runtime", $QuakeBase, $Map, [string]$Frames, "-game", $Game) "bp075-079r2-runtime-validation.log")
  } else {
  ...
  if ($CanUseGame) {
    $CoreA = Join-Path $Build "bp075-079r2-core-assets-a.log"
    $CoreB = Join-Path $Build "bp075-079r2-core-assets-b.log"
  ...
  if (-not $SkipAudioEvidence -and $CanUseGame) {
    ...
    $A = Join-Path $Build "bp075-079r2-audio-a.log"; $B = Join-Path $Build "bp075-079r2-audio-b.log"
  ...
  if (-not $SkipTraceValidation -and $CanUseGame) {
    $TraceDir = Join-Path $Build "bp075-079r2-traces"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $TraceDir; New-Item -ItemType Directory -Force -Path $TraceDir | Out-Null
    $PrefixA = Join-Path $TraceDir "run-a"; $PrefixB = Join-Path $TraceDir "run-b"
    [void](Run-Logged "compatibility trace A" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixA, "-game", $Game) "bp075-079r2-trace-a.log")
    [void](Run-Logged "compatibility trace B" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixB, "-game", $Game) "bp075-079r2-trace-b.log")
    [void](Run-Logged "byte-identical trace comparison" $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", (Join-Path $Build "bp075-079r2-trace-comparison.json")) "bp075-079r2-trace-comparison.log")
  ...
  if (-not $SkipRenderEvidence -and $CanUseGame) {
    $EvidenceDir = Join-Path $Build "bp075-079r2-render"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $EvidenceDir; New-Item -ItemType Directory -Force -Path $EvidenceDir | Out-Null
    $EA = Join-Path $EvidenceDir "a"; $EB = Join-Path $EvidenceDir "b"
    [void](Run-Logged "render evidence A" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EA, "-game", $Game) "bp075-079r2-render-a.log")
    [void](Run-Logged "render evidence B" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EB, "-game", $Game) "bp075-079r2-render-b.log")
    [void](Run-Logged "byte-identical render evidence" $PythonExe @($RenderComparator, ("{0}.tga" -f $EA), ("{0}.tga" -f $EB), "--require-exact", "--json-out", (Join-Path $Build "bp075-079r2-render-comparison.json")) "bp075-079r2-render-comparison.log")
  ...
  if ($NetworkTests) { [void](Run-Logged "Winsock UDP loopback smoke" $GameExe @("--udp-smoke", "2000") "bp075-079r2-udp-smoke.log" "result=PASS") }
  ...
  Add-Step "MiniQuake BP-075-079 acceptance" "PASS" 0 "all required gates passed"
} catch {
...
if (-not [string]::IsNullOrWhiteSpace($Failure)) {
  Write-Host "MiniQuake BP-075-079R2 acceptance test: FAIL" -ForegroundColor Red
  Write-Host "Run .\COLLECT_RESULTS.ps1 and upload the generated ZIP."
  exit 1
}
Write-Host "MiniQuake BP-075-079R2 acceptance test: PASS" -ForegroundColor Green
