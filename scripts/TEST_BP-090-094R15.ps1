...
param(
  ...
  [int]$OriginalServerReadyTimeoutMs = 180000,
  [ValidateRange(1, 1000000)]
...
$BlockId = "BP-090-094"
$DeliveryRevision = "BP-090-094R15"
$DeliveryParent = "BP-090-094R14"
...
$TranscriptPath = Join-Path $Build ("bp090-094r15-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $Build "bp090-094r15-test-summary.json"
$Steps = New-Object System.Collections.Generic.List[object]
$FirewallReportPath = Join-Path $Build "bp090-094r15-temporary-firewall-rules.json"
$FirewallRulePrefix = "MiniQuake-BP090094R15-Temporary"
$FirewallDisplayGroup = "MiniQuake BP-090-094R15 temporary loopback interop"
$FirewallRuleNames = @()
$FirewallRulesInstalled = $false
$FirewallCleanupStatus = "not_installed"
$FirewallSetupStatus = "not_requested"
$FirewallSetupError = ""
$OriginalReferenceSourceKind = ""
$OriginalReferenceSourcePath = ""
$OriginalReferenceResolution = ""
$OriginalInputReportPath = Join-Path $Build "bp090-094r15-original-reference-input.json"
...
function Test-PathInsideProject([string]$CandidatePath) {
...
function Resolve-OriginalReferenceInput() {
  $NeedReference = -not $SkipOriginalInterop -or -not $SkipOriginalVisualReference
  if (-not $NeedReference) {
    $script:OriginalReferenceSourceKind = "not_required"
    $script:OriginalReferenceSourcePath = ""
    $script:OriginalReferenceResolution = "not_required"
    return
  }

  if (-not [string]::IsNullOrWhiteSpace($OriginalQuakeSourceArchive) -and
      -not [string]::IsNullOrWhiteSpace($OriginalGLQuakeExe)) {
    throw "Specify exactly one of -OriginalQuakeSourceArchive or -OriginalGLQuakeExe."
  }

  $ArchiveCandidate = [string]$OriginalQuakeSourceArchive
  $ExeCandidate = [string]$OriginalGLQuakeExe
  $Resolution = "explicit"

  if ([string]::IsNullOrWhiteSpace($ArchiveCandidate) -and
      [string]::IsNullOrWhiteSpace($ExeCandidate)) {
    if (-not [string]::IsNullOrWhiteSpace($env:MINIQUAKE_ORIGINAL_SOURCE)) {
      $ArchiveCandidate = [string]$env:MINIQUAKE_ORIGINAL_SOURCE
      $Resolution = "environment_archive"
    } elseif (-not [string]::IsNullOrWhiteSpace($env:MINIQUAKE_ORIGINAL_EXE)) {
      $ExeCandidate = [string]$env:MINIQUAKE_ORIGINAL_EXE
      $Resolution = "environment_executable"
    } else {
      $DownloadsArchive = Join-Path $env:USERPROFILE "Downloads\OriginalQuakeSourceCode.zip"
      $ParentArchive = Join-Path (Split-Path -Parent $Root) "OriginalQuakeSourceCode.zip"
      $ProjectArchive = Join-Path $Root "OriginalQuakeSourceCode.zip"

      if (Test-Path -LiteralPath $DownloadsArchive -PathType Leaf) {
        $ArchiveCandidate = $DownloadsArchive
        $Resolution = "downloads_archive"
      } elseif (Test-Path -LiteralPath $ParentArchive -PathType Leaf) {
        $ArchiveCandidate = $ParentArchive
        $Resolution = "parent_archive"
      } elseif (Test-Path -LiteralPath $ProjectArchive -PathType Leaf) {
        throw ("OriginalQuakeSourceCode.zip is inside the MiniQuake project. " +
          "Move it outside the project, for example to: " + $DownloadsArchive)
      }
    }
  }
  ...
  if ([string]::IsNullOrWhiteSpace($ArchiveCandidate) -and
      [string]::IsNullOrWhiteSpace($ExeCandidate)) {
    throw ("Pass -OriginalQuakeSourceArchive PATH or -OriginalGLQuakeExe PATH. " +
      "R11 also checks the Downloads folder and MINIQUAKE_ORIGINAL_SOURCE.")
  }

  if (-not [string]::IsNullOrWhiteSpace($ArchiveCandidate)) {
    $Resolved = (Resolve-Path -LiteralPath $ArchiveCandidate -ErrorAction Stop).Path
    if (-not (Test-Path -LiteralPath $Resolved -PathType Leaf)) {
      throw "original source archive is not a file: $Resolved"
    }
    if (Test-PathInsideProject $Resolved) {
      throw "original source archive must remain outside the MiniQuake project: $Resolved"
    }

    $script:OriginalQuakeSourceArchive = $Resolved
    $script:OriginalGLQuakeExe = ""
    $script:OriginalReferenceSourceKind = "archive"
    $script:OriginalReferenceSourcePath = $Resolved
    $script:OriginalReferenceResolution = $Resolution
    $env:MINIQUAKE_ORIGINAL_SOURCE = $Resolved
    $env:MINIQUAKE_ORIGINAL_EXE = $null

    $script:ScriptInvocationParameters["OriginalQuakeSourceArchive"] = $Resolved
    [void]$script:ScriptInvocationParameters.Remove("OriginalGLQuakeExe")
  } else {
    $Resolved = (Resolve-Path -LiteralPath $ExeCandidate -ErrorAction Stop).Path
    if (-not (Test-Path -LiteralPath $Resolved -PathType Leaf)) {
      throw "original GLQuake executable is not a file: $Resolved"
    }
    if (Test-PathInsideProject $Resolved) {
      throw "original GLQuake executable must remain outside the MiniQuake project: $Resolved"
    }

    $script:OriginalQuakeSourceArchive = ""
    $script:OriginalGLQuakeExe = $Resolved
    $script:OriginalReferenceSourceKind = "executable"
    $script:OriginalReferenceSourcePath = $Resolved
    $script:OriginalReferenceResolution = $Resolution
    $env:MINIQUAKE_ORIGINAL_SOURCE = $null
    $env:MINIQUAKE_ORIGINAL_EXE = $Resolved

    $script:ScriptInvocationParameters["OriginalGLQuakeExe"] = $Resolved
    [void]$script:ScriptInvocationParameters.Remove("OriginalQuakeSourceArchive")
  }

  Write-Host (
    "[MiniQuake] original reference input resolved: kind={0} resolution={1} path={2}" -f
      $script:OriginalReferenceSourceKind,
      $script:OriginalReferenceResolution,
      $script:OriginalReferenceSourcePath
  ) -ForegroundColor Cyan
}

function Write-OriginalReferenceInputReport(
  [string]$Status,
  [string]$Selector,
  [string[]]$Arguments
) {
  ...
  foreach ($Argument in $Arguments) {
    if ($Argument -eq $script:OriginalReferenceSourcePath) {
      $SafeArguments += "<external-reference-path>"
    } else {
  ...
  [ordered]@{
    ...
    status = $Status
    source_kind = $script:OriginalReferenceSourceKind
    source_resolution = $script:OriginalReferenceResolution
    source_path = $script:OriginalReferenceSourcePath
    source_exists = [bool](Test-Path -LiteralPath $script:OriginalReferenceSourcePath -PathType Leaf)
    source_inside_project = [bool](Test-PathInsideProject $script:OriginalReferenceSourcePath)
    selector = $Selector
    selector_count = @(
      $Arguments | Where-Object { $_ -eq "--archive" -or $_ -eq "--exe" }
    ).Count
    argument_names = $SafeArguments
    environment_archive_set = -not [string]::IsNullOrWhiteSpace($env:MINIQUAKE_ORIGINAL_SOURCE)
    environment_executable_set = -not [string]::IsNullOrWhiteSpace($env:MINIQUAKE_ORIGINAL_EXE)
  } | ConvertTo-Json -Depth 6 |
    Set-Content -LiteralPath $OriginalInputReportPath -Encoding UTF8
}
...
function Run-NetworkEvidencePair([string]$Suffix, [int]$Port) {
  $ServerOut = Join-Path $Build ("bp090-094r15-network-server-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp090-094r15-network-server-{0}.err.log" -f $Suffix)
  $ClientLog = Join-Path $Build ("bp090-094r15-network-client-{0}.log" -f $Suffix)
  $PairJson = Join-Path $Build ("bp090-094r15-network-pair-{0}.json" -f $Suffix)
...
function Read-QuakeCString([byte[]]$Data, [int]$Offset) {
  $DataLength = 0
  if ($null -ne $Data) { $DataLength = $Data.Length }
  if ($null -eq $Data -or $Offset -lt 0 -or $Offset -ge $DataLength) {
    throw ("invalid Quake string offset {0} for packet length {1}" -f $Offset, $DataLength)
  }
  $End = $Offset
  while ($End -lt $Data.Length -and $Data[$End] -ne 0) { $End = $End + 1 }
  if ($End -ge $Data.Length) { throw "unterminated Quake string in control packet" }
  $Value = [Text.Encoding]::ASCII.GetString($Data, $Offset, $End - $Offset)
  return [pscustomobject]@{
    value = $Value
    next_offset = $End + 1
  }
...
function Get-AvailableLoopbackUdpPort([int]$Minimum, [int]$Maximum) {
  if ($Minimum -lt 1 -or $Maximum -gt 65535 -or $Minimum -gt $Maximum) {
    throw ("invalid UDP port range {0}..{1}" -f $Minimum, $Maximum)
  }
  for ($Attempt = 1; $Attempt -le 128; $Attempt++) {
    $Candidate = Get-Random -Minimum $Minimum -Maximum ($Maximum + 1)
    $Probe = $null
    try {
      $Probe = New-Object System.Net.Sockets.UdpClient
      $Endpoint = New-Object System.Net.IPEndPoint -ArgumentList ([Net.IPAddress]::Parse('127.0.0.1')), $Candidate
      $Probe.Client.Bind($Endpoint)
      return $Candidate
    } catch {
      # A concurrently used port is simply skipped.
    } finally {
      if ($null -ne $Probe) { try { $Probe.Close() } catch { } }
    }
  }
  throw ("INFRA_FAILURE: could not find a free loopback UDP port in range {0}..{1}" -f $Minimum, $Maximum)
}

function Wait-OriginalServerControlReady(
  [pscustomobject]$Handle,
  [string]$Suffix,
  [int]$Port,
  [int]$TimeoutMilliseconds,
  ...
  # Quake control packet: NETFLAG_CTL | length(12), CCREQ_SERVER_INFO,
  # "QUAKE\\0", NET_PROTOCOL_VERSION(3).  R13 keeps this exact source
  # endpoint alive until after the original process stops.  This avoids a
  # closed-readiness-port/late-reply transition immediately before the real
  # Protocol-3 connect socket is opened.
  [byte[]]$Request = @(0x80, 0x00, 0x00, 0x0c, 0x02, 0x51, 0x55, 0x41, 0x4b, 0x45, 0x00, 0x03)
  ...
  Write-Host ("[MiniQuake] waiting for original GLQuake server {0} Protocol-3 readiness on 127.0.0.1:{1} (timeout {2} ms)" -f $Suffix, $Port, $TimeoutMilliseconds) -ForegroundColor Cyan

  try {
    # Prefer observing the requested process/port binding before sending any
    # control packet.  The probe remains optional because Get-NetUDPEndpoint is
    # not present on every supported PowerShell installation.
    $EndpointStarted = [DateTime]::UtcNow
    $EndpointProbeCommand = Get-Command Get-NetUDPEndpoint -ErrorAction SilentlyContinue
    $EndpointProbeAvailable = $null -ne $EndpointProbeCommand
    if ($EndpointProbeAvailable) {
      $EndpointDeadline = [DateTime]::UtcNow.AddMilliseconds([Math]::Min(30000, $TimeoutMilliseconds))
      while ([DateTime]::UtcNow -lt $EndpointDeadline -and -not $EndpointObserved) {
        if ($Handle.process.HasExited) { break }
        try {
          $Matches = @(
            Get-NetUDPEndpoint -OwningProcess $Handle.process.Id -ErrorAction SilentlyContinue |
              Where-Object { [int]$_.LocalPort -eq $Port }
          )
          if ($Matches.Count -gt 0) { $EndpointObserved = $true; break }
        } catch { }
        Start-Sleep -Milliseconds 100
      }
    } else {
      # Give the GUI process a minimal scheduling window before the first UDP
      # packet when endpoint enumeration is unavailable.
      Start-Sleep -Milliseconds 500
    }
    $EndpointWaitMs = [int]([DateTime]::UtcNow - $EndpointStarted).TotalMilliseconds

    $Udp = New-Object System.Net.Sockets.UdpClient
    $Bind = New-Object System.Net.IPEndPoint -ArgumentList ([Net.IPAddress]::Parse('127.0.0.1')), 0
    $Udp.Client.Bind($Bind)
    $Udp.Client.ReceiveTimeout = 750
    $GuardLocalPort = ([System.Net.IPEndPoint]$Udp.Client.LocalEndPoint).Port
    $Target = New-Object System.Net.IPEndPoint -ArgumentList ([Net.IPAddress]::Parse('127.0.0.1')), $Port

    while ([DateTime]::UtcNow -lt $Deadline -and -not $Ready) {
      if ($Handle.process.HasExited) {
        $Handle.process.WaitForExit()
        $Handle.process.Refresh()
        $LastError = "original process exited with code " + [int]$Handle.process.ExitCode
        break
      }

      $Attempts = $Attempts + 1
      try {
        $Written = $Udp.Send($Request, $Request.Length, $Target)
        if ($Written -ne $Request.Length) {
          throw ("readiness request was partially sent: {0}/{1}" -f $Written, $Request.Length)
        }
        $Remote = New-Object System.Net.IPEndPoint -ArgumentList ([Net.IPAddress]::Any), 0
        [byte[]]$Response = $Udp.Receive([ref]$Remote)
        $LastRemote = $Remote.Address.ToString() + ':' + $Remote.Port
        $ResponseBytes = $Response.Length
        if ($Remote.Address.ToString() -ne '127.0.0.1' -or $Remote.Port -ne $Port) {
          throw ("server-info response came from unexpected endpoint {0}" -f $LastRemote)
        }

        if ($Response.Length -lt 8) { throw "server-info response is too short" }
        $WireLength = ([int]$Response[2] -shl 8) -bor [int]$Response[3]
        if ($Response[0] -ne 0x80 -or $Response[1] -ne 0x00) { throw "response is not NETFLAG_CTL" }
        if ($WireLength -ne $Response.Length) { throw ("response length field {0} does not match {1}" -f $WireLength, $Response.Length) }
        if ($Response[4] -ne 0x83) { throw ("response command is 0x{0:x2}, expected CCREP_SERVER_INFO" -f $Response[4]) }

        $Cursor = 5
        $Field = Read-QuakeCString -Data $Response -Offset $Cursor
        $ServerAddress = [string]$Field.value
        $Cursor = [int]$Field.next_offset
        $Field = Read-QuakeCString -Data $Response -Offset $Cursor
        $HostName = [string]$Field.value
        $Cursor = [int]$Field.next_offset
        $Field = Read-QuakeCString -Data $Response -Offset $Cursor
        $MapName = [string]$Field.value
        $Cursor = [int]$Field.next_offset
        if ($Cursor + 3 -gt $Response.Length) { throw "server-info response is truncated after map name" }
        $CurrentPlayers = [int]$Response[$Cursor]
        $MaximumPlayers = [int]$Response[$Cursor + 1]
        $Protocol = [int]$Response[$Cursor + 2]
        if ($Protocol -ne 3) { throw ("server-info control protocol is {0}, expected 3" -f $Protocol) }
        if ([string]::IsNullOrWhiteSpace($MapName)) { throw "server-info response has no active map name" }
        $Ready = $true
      ...
      if (-not $Ready -and [DateTime]::UtcNow -lt $Deadline) {
        Start-Sleep -Milliseconds 250
      }
    }
    if ($Ready) { $KeepGuardOpen = $true }
  } finally {
    if ($null -ne $Udp -and -not $KeepGuardOpen) { try { $Udp.Close() } catch { } }
  }
  ...
  $ExitCode = $null
  try {
    $ProcessAlive = -not $Handle.process.HasExited
    if (-not $ProcessAlive) {
      $Handle.process.WaitForExit()
      $Handle.process.Refresh()
  ...
  $Report = [ordered]@{
    ...
    evidence = 'protocol3_server_info_response_with_open_guard'
    target_address = '127.0.0.1'
    target_port = $Port
    timeout_ms = $TimeoutMilliseconds
    elapsed_ms = $Elapsed
    endpoint_probe_available = [bool]$EndpointProbeAvailable
    endpoint_observed_before_probe = [bool]$EndpointObserved
    endpoint_wait_ms = $EndpointWaitMs
    probe_attempts = $Attempts
    response_bytes = $ResponseBytes
    response_remote = $LastRemote
    ...
    guard_local_address = '127.0.0.1'
    guard_local_port = $GuardLocalPort
    guard_socket_open = [bool]$Ready
    guard_close_order = 'after_original_process_stop'
    process_id = $Handle.process.Id
    process_alive = [bool]$ProcessAlive
    process_exit_code = $ExitCode
    process_udp_endpoints = $Endpoints
    last_error = $LastError
  }
  $Report | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $ReportPath -Encoding UTF8

  if (-not $Ready) {
    throw ("INFRA_FAILURE: original GLQuake server {0} did not answer a Protocol-3 server-info probe on 127.0.0.1:{1} within {2} ms; report={3}" -f $Suffix, $Port, $TimeoutMilliseconds, $ReportPath)
  }

  Write-Host ("[MiniQuake] original GLQuake server {0} ready: probes={1} elapsed_ms={2} map={3} host={4} protocol={5} guard=127.0.0.1:{6}" -f $Suffix, $Attempts, $Elapsed, $MapName, $HostName, $Protocol, $GuardLocalPort) -ForegroundColor Green
  $Result = [pscustomobject]$Report
  Add-Member -InputObject $Result -NotePropertyName guard_socket -NotePropertyValue $Udp
  return $Result
}
...
function Run-OriginalServerMiniClientPair([string]$Suffix, [int]$Port) {
  ...
  $OriginalServerOut = Join-Path $Build ("bp090-094r15-original-server-{0}-stdout.log" -f $Suffix)
  $OriginalServerErr = Join-Path $Build ("bp090-094r15-original-server-{0}-stderr.log" -f $Suffix)
  $ProcessReportPath = Join-Path $Build ("bp090-094r15-original-server-{0}-process.json" -f $Suffix)
  $ReadinessReportPath = Join-Path $Build ("bp090-094r15-original-server-{0}-readiness.json" -f $Suffix)
  Remove-Item -Force -ErrorAction SilentlyContinue $ReadinessReportPath
  ...
  try {
    # A cold 1997 GLQuake startup on a modern driver may spend tens of seconds
    # creating its WGL context and uploading textures.  Do not launch the full
    # MiniQuake interop client until a real CCREP_SERVER_INFO proves that the
    # requested UDP socket and network event loop are ready.
    $Readiness = Wait-OriginalServerControlReady -Handle $OriginalServer -Suffix $Suffix -Port $Port -TimeoutMilliseconds $OriginalServerReadyTimeoutMs -ReportPath $ReadinessReportPath
    $Deadline = [DateTime]::UtcNow.AddMilliseconds(90000)
    ...
    while ([DateTime]::UtcNow -lt $Deadline -and -not $SignonComplete) {
      ...
      $Attempt = $Attempt + 1
      $AttemptPrefix = Join-Path $Build ("bp090-094r15-miniquake-client-original-server-{0}-attempt-{1}" -f $Suffix, $Attempt)
      $AttemptLogName = "bp090-094r15-miniquake-client-original-server-{0}-attempt-{1}.log" -f $Suffix, $Attempt
      ...
      if ($LastClientCode -eq 0 -and $LastClientText -match 'result=PASS' -and (Test-Path -LiteralPath $AttemptSummaryPath -PathType Leaf)) {
        ...
        $HasTargetUdpProvenance = (
          $LastClientText -match 'network_provenance=target_udp' -and
          $LastClientText -match 'transport=udp' -and
          $LastClientText -match 'local_server_active=false' -and
          $LastClientText -match 'local_authoritative=false' -and
          $LastClientText -match 'demo_playback=false'
        )
        if ($AttemptSummary.success -and $AttemptSummary.connected -and $AttemptSummary.spawned -and [int]$AttemptSummary.signon -eq 4 -and [int]$AttemptSummary.protocol -eq 15 -and $HasTargetUdpProvenance) {
          $SignonComplete = $true
    ...
    $Normalized = Join-Path $Build ("bp090-094r15-original-server-mini-client-{0}-normalized.json" -f $Suffix)
    ...
    try {
      [ordered]@{
        ...
        readiness_report = [IO.Path]::GetFileName($ReadinessReportPath)
        readiness_status = if ($null -eq $Readiness) { 'not_completed' } else { [string]$Readiness.status }
        readiness_probe_attempts = if ($null -eq $Readiness) { 0 } else { [int]$Readiness.probe_attempts }
        readiness_elapsed_ms = if ($null -eq $Readiness) { 0 } else { [int]$Readiness.elapsed_ms }
        readiness_map = if ($null -eq $Readiness) { '' } else { [string]$Readiness.map }
        readiness_control_protocol = if ($null -eq $Readiness) { -1 } else { [int]$Readiness.control_protocol }
        readiness_guard_local_port = if ($null -eq $Readiness) { 0 } else { [int]$Readiness.guard_local_port }
        guard_socket_open_during_signon = if ($null -eq $Readiness) { $false } else { $null -ne $Readiness.guard_socket }
        guard_close_order = 'after_original_process_stop'
        pre_fallback_connect_required = $true
        attempts = $Attempt
    ...
    if ($null -ne $Readiness -and $null -ne $Readiness.guard_socket) {
      try { $Readiness.guard_socket.Close() } catch { }
      Write-Host ("[MiniQuake] original GLQuake readiness guard {0} closed after server stop" -f $Suffix) -ForegroundColor DarkGray
    }
  }
}

function Run-MiniServerOriginalClientPair([string]$Suffix, [int]$Port) {
  $Prefix = Join-Path $Build ("bp090-094r15-miniquake-server-original-client-{0}" -f $Suffix)
  $ReadyPath = $Prefix + '-ready.json'
  $SummaryPath = $Prefix + '-summary.json'
  $ServerOut = Join-Path $Build ("bp090-094r15-miniquake-server-original-client-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp090-094r15-miniquake-server-original-client-{0}.err.log" -f $Suffix)
  $ProcessReportPath = Join-Path $Build ("bp090-094r15-original-client-{0}-process.json" -f $Suffix)
  Remove-Item -Force -ErrorAction SilentlyContinue $ReadyPath, $SummaryPath, $ProcessReportPath
  ...
  $ConfigPath = Join-Path $GameDirectory $ConfigName
  $ClientAliveAtSignon = $false
  $ClientExitCode = $null
  $ClientExitedBeforeSignon = $false
  $ServerResult = $null
  $FailureText = ''
  try {
    ...
    # Original Quake's Cmd_TokenizeString uses COM_Parse, where ':' is a
    # standalone punctuation token. Host_Connect_f consumes only Cmd_Argv(1),
    # so a host-colon-port console token silently drops PORT and targets 26000.
    # Route the remote port through NET_Init's historical -port option and keep
    # the console connect token free of a colon.
    [IO.File]::WriteAllLines(
      $ConfigPath,
      @('unbindall', 'developer 1', 'name original_ref', 'color 0 0', 'connect 127.0.0.1'),
      [Text.Encoding]::ASCII
    )
    ...
    Write-Host ("[MiniQuake] starting original GLQuake client {0}: connect 127.0.0.1 via -port {1}" -f $Suffix, $Port) -ForegroundColor Cyan
    ...
    # Monitor both processes and the protocol summary. This preserves the
    # foreground live output while giving a precise client status on failure.
    $Deadline = [DateTime]::UtcNow.AddSeconds(60)
    while ([DateTime]::UtcNow -lt $Deadline) {
      if (Test-Path -LiteralPath $SummaryPath -PathType Leaf) { break }
      if ($OriginalClient.HasExited) {
        $OriginalClient.WaitForExit()
        $OriginalClient.Refresh()
        $ClientExitedBeforeSignon = $true
        $ClientExitCode = [int]$OriginalClient.ExitCode
        throw ("original GLQuake client {0} exited before completed signon: exit={1}" -f $Suffix, $ClientExitCode)
      }
    ...
    $Remaining = 5000
    if (-not $Server.process.HasExited -and -not (Test-Path -LiteralPath $SummaryPath -PathType Leaf)) {
      throw ("MiniQuake original-client server {0} timed out waiting for a Protocol-15 summary; original client alive={1}" -f $Suffix, (-not $OriginalClient.HasExited))
    }
    $ServerResult = Complete-BackgroundCapturedProcess -Handle $Server -TimeoutMilliseconds $Remaining -StdoutPath $ServerOut -StderrPath $ServerErr
    ...
    [ordered]@{
      ...
      client_exited_before_signon = [bool]$ClientExitedBeforeSignon
    ...
    $Normalized = Join-Path $Build ("bp090-094r15-mini-server-original-client-{0}-normalized.json" -f $Suffix)
    ...
    try {
      ...
      [ordered]@{
        ...
        remote_port = $Port
        config_path = $ConfigPath
        process_started = ($null -ne $OriginalClient)
        process_alive_at_failure = ($null -ne $OriginalClient -and -not $OriginalClient.HasExited)
        client_exited_before_signon = [bool]$ClientExitedBeforeSignon
        client_exit_code = $ClientExitCode
        summary_exists = (Test-Path -LiteralPath $SummaryPath -PathType Leaf)
        server_exited = [bool]$Server.process.HasExited
        failure = $FailureText
        status = 'FAIL'
      } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $ProcessReportPath -Encoding UTF8
    } catch { }
...
function Get-OriginalTgaState([string]$Path) {
  $Result = [ordered]@{ exists=$false; complete=$false; width=0; height=0; bits_per_pixel=0; image_type=0; color_map_type=0; id_length=0; actual_bytes=0; expected_bytes=0; sha256=''; last_write_utc=''; error='' }
  if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { return [pscustomobject]$Result }
  try {
    $Item=Get-Item -LiteralPath $Path -ErrorAction Stop; $Result.exists=$true; $Result.actual_bytes=[int64]$Item.Length; $Result.last_write_utc=$Item.LastWriteTimeUtc.ToString('o')
    if ($Item.Length -lt 18) { return [pscustomobject]$Result }
    $Bytes=[IO.File]::ReadAllBytes($Path); if ($Bytes.Length -lt 18) { return [pscustomobject]$Result }
    $Id=[int]$Bytes[0]; $Cmap=[int]$Bytes[1]; $Type=[int]$Bytes[2]
    $Width=[int]$Bytes[12]+256*[int]$Bytes[13]; $Height=[int]$Bytes[14]+256*[int]$Bytes[15]; $Bits=[int]$Bytes[16]
    $Result.id_length=$Id; $Result.color_map_type=$Cmap; $Result.image_type=$Type; $Result.width=$Width; $Result.height=$Height; $Result.bits_per_pixel=$Bits
    if ($Cmap -ne 0 -or $Type -ne 2 -or $Bits -ne 24 -or $Width -le 0 -or $Height -le 0) { $Result.error='unsupported or invalid TGA header'; return [pscustomobject]$Result }
    $Expected=[int64](18+$Id)+[int64]$Width*[int64]$Height*3; $Result.expected_bytes=$Expected
    if ([int64]$Bytes.Length -ne $Expected) { return [pscustomobject]$Result }
    $Result.sha256=(Get-FileHash -LiteralPath $Path -Algorithm SHA256).Hash.ToLowerInvariant(); $Result.complete=$true
  } catch { $Result.error=$_.Exception.Message }
  return [pscustomobject]$Result
}

function Wait-OriginalTgaComplete([string]$GameDirectory,[System.Diagnostics.Process]$Process,[int]$TimeoutSeconds=90) {
  $Deadline=[DateTime]::UtcNow.AddSeconds($TimeoutSeconds); $StablePolls=0; $LastHash=''; $LastState=$null; $Path=''; $ExitedBeforeComplete=$false; $ExitCodeBeforeComplete=$null
  while ([DateTime]::UtcNow -lt $Deadline) {
    if ([string]::IsNullOrWhiteSpace($Path)) {
      ...
      if ($null -ne $Found) { $Path=$Found.FullName }
    }
    if (-not [string]::IsNullOrWhiteSpace($Path)) {
      $State=Get-OriginalTgaState -Path $Path; $LastState=$State
      if ($State.complete) {
        if ($State.sha256 -eq $LastHash) { $StablePolls++ } else { $LastHash=$State.sha256; $StablePolls=1 }
        if ($StablePolls -ge 3) {
          $ExitedAfterCapture=$Process.HasExited; $ExitCodeAfterCapture=$null
          if (-not $ExitedAfterCapture) { try { $ExitedAfterCapture=$Process.WaitForExit(5000) } catch { } }
          if ($ExitedAfterCapture) { try { $Process.WaitForExit(); $Process.Refresh(); $ExitCodeAfterCapture=[int]$Process.ExitCode } catch { } }
          $FinalState=Get-OriginalTgaState -Path $Path
          if ($FinalState.complete -and $FinalState.sha256 -eq $LastHash) {
            return [pscustomobject]@{ path=$Path; state=$FinalState; stable_polls=$StablePolls; process_exited_after_capture=[bool]$ExitedAfterCapture; process_exit_code_after_capture=$ExitCodeAfterCapture; process_exited_before_complete=[bool]$ExitedBeforeComplete; process_exit_code_before_complete=$ExitCodeBeforeComplete }
          }
          $LastHash=''; $StablePolls=0
        }
      } else { $StablePolls=0; $LastHash='' }
    }
    if ($Process.HasExited) {
      try { $Process.WaitForExit(); $Process.Refresh(); $ExitedBeforeComplete=$true; $ExitCodeBeforeComplete=[int]$Process.ExitCode } catch { }
      if ($null -eq $LastState -or -not $LastState.complete) { break }
    }
    Start-Sleep -Milliseconds 100
  }
  $Detail=if ($null -eq $LastState) { 'no screenshot file observed' } else { "exists=$($LastState.exists) complete=$($LastState.complete) actual=$($LastState.actual_bytes) expected=$($LastState.expected_bytes) error=$($LastState.error)" }
  throw "original GLQuake screenshot did not become a complete stable TGA: $Detail"
}

function Run-OriginalVisualCapture([string]$Demo,[int]$Frame,[string]$Suffix,[string]$ScenarioRoot) {
  $GameName="bp093_{0}_{1}" -f $Demo,$Suffix; $GameDirectory=New-OriginalGameDirectory $GameName; $ConfigName='visual.cfg'
  $ProcessReportPath=Join-Path $Build ("bp090-094r15-original-visual-{0}-{1}-process.json" -f $Demo,$Suffix)
  Write-OriginalVisualConfig -Path (Join-Path $GameDirectory $ConfigName) -Demo $Demo -Frame $Frame
  $Process=Start-GuiProcess -Executable $OriginalExe -Arguments @('-basedir',$OriginalBase,'-game',$GameName,'-window','-width','640','-height','480','-gamma','1','-nosound','-nocdaudio','-nomouse','-nojoy','-noudp','-noipx','+exec',$ConfigName) -WorkingDirectory (Join-Path $OriginalStage 'bin') -Label ("original visual {0}-{1}" -f $Demo,$Suffix)
  try {
    $Completed=Wait-OriginalTgaComplete -GameDirectory $GameDirectory -Process $Process -TimeoutSeconds 90
    $Screenshot=[string]$Completed.path; $SourceState=$Completed.state; $Target=Join-Path $ScenarioRoot ("original-{0}.tga" -f $Suffix)
    Copy-Item -LiteralPath $Screenshot -Destination $Target -Force; $TargetState=Get-OriginalTgaState -Path $Target
    if (-not $TargetState.complete -or $TargetState.sha256 -ne $SourceState.sha256) { throw 'copied original GLQuake TGA failed completion/hash verification' }
    [ordered]@{ schema_version=2; demo=$Demo; frame=$Frame; suffix=$Suffix; condebug_enabled=$false; network_scope='disabled'; bind_address=''; firewall_prompt_expected=$false; evidence_source='original_tga_screenshot'; screenshot_produced=$true; screenshot_name=[IO.Path]::GetFileName($Screenshot); capture_completion='validated_tga_size_hash_stability'; simulation_timestep='fixed_host_framerate'; original_host_framerate=0.02; miniquake_frame_step=0.02; tga_width=[int]$SourceState.width; tga_height=[int]$SourceState.height; tga_bits_per_pixel=[int]$SourceState.bits_per_pixel; expected_tga_bytes=[int64]$SourceState.expected_bytes; actual_tga_bytes=[int64]$SourceState.actual_bytes; source_sha256=[string]$SourceState.sha256; copied_sha256=[string]$TargetState.sha256; stable_polls=[int]$Completed.stable_polls; process_exited_after_capture=[bool]$Completed.process_exited_after_capture; process_exit_code_after_capture=$Completed.process_exit_code_after_capture; process_exited_before_complete=[bool]$Completed.process_exited_before_complete; process_exit_code_before_complete=$Completed.process_exit_code_before_complete } | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $ProcessReportPath -Encoding UTF8
    return $Target
  } finally { Stop-GuiProcess $Process }
}
...
if ($OriginalReferenceSourceKind -ne "not_required") {
  Write-OriginalReferenceInputReport -Status "RESOLVED" -Selector "" -Arguments @()
}
Start-Transcript -Path $TranscriptPath -Force | Out-Null
$Failure = ""
try {
  Write-Host "MiniQuake BP-090-094R15 acceptance test"
  ...
  if (-not $SkipBuild) {
    ...
    [void](Run-Logged "single cumulative build and unit-test suite" $PowerShellExe $Arguments "bp090-094r15-build-child.log" "MiniQuake BP-094 external compatibility closure tests passed: 24")
  } else {
  ...
  $Version = Run-Logged "package identity" $GameExe @("--version") "bp090-094r15-version.txt" "Compatibility release status: compat_109_release_candidate_v1"
  ...
  $OriginalStage = Join-Path $Build 'bp090-094r15-original-reference'
  $OriginalExe = ''
  $OriginalBase = ''
  if ($NeedOriginalReference) {
    ...
    if ([string]::IsNullOrWhiteSpace($OriginalReferenceSourceKind) -or
        [string]::IsNullOrWhiteSpace($OriginalReferenceSourcePath)) {
      throw "original reference input was not resolved before the build"
    }

    $PrepareList = [System.Collections.Generic.List[string]]::new()
    [void]$PrepareList.Add($OriginalPrepareTool)
    $PrepareSelector = ""

    if ($OriginalReferenceSourceKind -eq "archive") {
      $PrepareSelector = "--archive"
      [void]$PrepareList.Add("--archive")
      [void]$PrepareList.Add($OriginalReferenceSourcePath)
    } elseif ($OriginalReferenceSourceKind -eq "executable") {
      $PrepareSelector = "--exe"
      [void]$PrepareList.Add("--exe")
      [void]$PrepareList.Add($OriginalReferenceSourcePath)
    } else {
      throw ("unsupported original reference source kind: " + $OriginalReferenceSourceKind)
    }

    [void]$PrepareList.Add("--quake-base")
    [void]$PrepareList.Add($QuakeBase)
    [void]$PrepareList.Add("--stage")
    [void]$PrepareList.Add($OriginalStage)
    [void]$PrepareList.Add("--json")
    [void]$PrepareList.Add((Join-Path $Build "bp090-094r15-original-reference-provenance.json"))

    $PrepareArguments = $PrepareList.ToArray()
    $SelectorCount = @(
      $PrepareArguments | Where-Object { $_ -eq "--archive" -or $_ -eq "--exe" }
    ).Count

    if ($SelectorCount -ne 1) {
      Write-OriginalReferenceInputReport `
        -Status "INVALID" `
        -Selector $PrepareSelector `
        -Arguments $PrepareArguments
      throw ("original reference preparation requires exactly one source selector, got " + $SelectorCount)
    }
    ...
    [void](Run-Logged `
      ...
      -Arguments $PrepareArguments `
      -LogName "bp090-094r15-original-reference.log" `
      -RequiredMarker "result=PASS")
  ...
  if (-not $SkipOriginalInterop) {
    $A1 = Run-OriginalServerMiniClientPair -Suffix 'a' -Port (Get-AvailableLoopbackUdpPort -Minimum 30000 -Maximum 36000)
    $A2 = Run-OriginalServerMiniClientPair -Suffix 'b' -Port (Get-AvailableLoopbackUdpPort -Minimum 36001 -Maximum 42000)
    ...
    $B1 = Run-MiniServerOriginalClientPair -Suffix 'a' -Port (Get-AvailableLoopbackUdpPort -Minimum 42001 -Maximum 50000)
    $B2 = Run-MiniServerOriginalClientPair -Suffix 'b' -Port (Get-AvailableLoopbackUdpPort -Minimum 50001 -Maximum 60000)
  ...
  if (-not $SkipOriginalVisualReference) {
    $VisualRoot = Join-Path $Build 'bp090-094r15-original-visual'
    ...
    foreach ($DemoName in @('demo1', 'demo2', 'demo3')) {
      ...
      $OriginalHashA=(Get-FileHash -LiteralPath $OriginalA -Algorithm SHA256).Hash.ToLowerInvariant(); $OriginalHashB=(Get-FileHash -LiteralPath $OriginalB -Algorithm SHA256).Hash.ToLowerInvariant(); $OriginalExact=($OriginalHashA -eq $OriginalHashB)
      $Candidates=@()
      for ($Offset=-2; $Offset -le 2; $Offset++) {
        $FrameNumber=256+$Offset; $Prefix=Join-Path $ScenarioRoot ("miniquake-{0}" -f $FrameNumber)
        [void](Run-Logged ("MiniQuake demo capture {0} frame {1}" -f $DemoName,$FrameNumber) $GameExe @('--render-demo-evidence',$QuakeBase,$DemoName,[string]$FrameNumber,$Prefix,'-game',$Game) ("bp090-094r15-{0}-frame-{1}.log" -f $DemoName,$FrameNumber) 'MiniQuake render evidence: PASS')
        $Candidates+=@('--candidate',("{0}:{1}.tga" -f $FrameNumber,$Prefix))
      }
      $VisualJson=Join-Path $ScenarioRoot 'comparison.json'
      $CompareArgs=@($OriginalVisualComparator,$OriginalA,'--original-alt',$OriginalB,'--scenario',$Scenario,'--min-reference-ssim','0.98','--min-ssim','0.95','--json-out',$VisualJson)+$Candidates
      [void](Run-Logged ("raw original visual ensemble comparison {0}" -f $Scenario) $PythonExe $CompareArgs ("bp090-094r15-{0}-comparison.log" -f $Scenario) 'result=PASS')
      $Comparison=Read-JsonFile $VisualJson; $ReferenceSsim=[double]$Comparison.reference_consistency.minimum_ssim
      if ($Comparison.reference_consistency.status -ne 'PASS' -or $ReferenceSsim -lt 0.98) { throw "original GLQuake reference runs are not sufficiently consistent for ${Scenario}: ssim=$ReferenceSsim" }
      if ($Comparison.status -ne 'PASS' -or [double]$Comparison.best.ssim_min -lt 0.95 -or $Comparison.normalization -ne 'none') { throw "original visual reference failed for $Scenario" }
      $ReferenceMode=if ($OriginalExact) { 'exact_pair' } else { 'bounded_dual_reference_ensemble' }
      Add-Step ("original GLQuake reference capture {0}" -f $Scenario) 'PASS' 0 ("mode={0} exact={1} reference_ssim={2} sha256_a={3} sha256_b={4}" -f $ReferenceMode,$OriginalExact,$ReferenceSsim,$OriginalHashA,$OriginalHashB)
      $VisualResults+=[ordered]@{ scenario=$Scenario; original_sha256_a=$OriginalHashA; original_sha256_b=$OriginalHashB; original_exact=[bool]$OriginalExact; original_reference_mode=$ReferenceMode; original_reference_ssim=$ReferenceSsim; best_frame=[int]$Comparison.best.frame; ssim=[double]$Comparison.best.ssim_min; ssim_mean=[double]$Comparison.best.ssim_mean; mae=[double]$Comparison.best.mae; psnr=[double]$Comparison.best.psnr; report=$VisualJson }
    }

    $VisualSummary = Join-Path $Build 'bp090-094r15-original-visual-summary.json'
    [ordered]@{ schema_version=3; status='PASS'; minimum_ssim=0.95; minimum_reference_ssim=0.98; reference_aggregation='minimum_ssim'; normalization='none'; simulation_timestep='fixed_0.02_seconds'; original_host_framerate=0.02; miniquake_frame_step=0.02; scenarios=$VisualResults } | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $VisualSummary -Encoding UTF8
  ...
  if ($CanUseGame) {
    ...
    [void](Run-Logged "installed Quake data validation" $GameExe @("--validate-game", $QuakeBase, $Map, "-game", $Game) "bp090-094r15-game-validation.log" "Validation result: PASS")
    [void](Run-Logged "headless runtime validation" $GameExe @("--validate-runtime", $QuakeBase, $Map, [string]$Frames, "-game", $Game) "bp090-094r15-runtime-validation.log")
  } else {
  ...
  if (-not $SkipMissionPackEvidence -and $CanUseGame) {
    foreach ($Mission in @("rogue", "hipnotic")) {
      ...
      $MissionFlag = "-" + $Mission
      [void](Run-Logged ("mission-pack {0} game validation" -f $Mission) $GameExe @("--validate-game", $QuakeBase, "start", $MissionFlag) ("bp090-094r15-{0}-validation.log" -f $Mission) "Validation result: PASS")
      [void](Run-Logged ("mission-pack {0} headless runtime" -f $Mission) $GameExe @("-basedir", $QuakeBase, $MissionFlag, "-headless", "-nosound", "-maxframes", "128", "+map", "start") ("bp090-094r15-{0}-runtime.log" -f $Mission))
    }
  } else {
  ...
  if ($CanUseGame) {
    $CoreA = Join-Path $Build "bp090-094r15-core-assets-a.log"
    $CoreB = Join-Path $Build "bp090-094r15-core-assets-b.log"
  ...
  if (-not $SkipAudioEvidence -and $CanUseGame) {
    ...
    $A = Join-Path $Build "bp090-094r15-audio-a.log"; $B = Join-Path $Build "bp090-094r15-audio-b.log"
  ...
  if (-not $SkipArtifactEvidence -and $CanUseGame) {
    $ArtifactA = Join-Path $Build "bp090-094r15-artifacts-a.log"
    $ArtifactB = Join-Path $Build "bp090-094r15-artifacts-b.log"
  ...
  if (-not $SkipTraceValidation -and $CanUseGame) {
    $TraceDir = Join-Path $Build "bp090-094r15-traces"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $TraceDir; New-Item -ItemType Directory -Force -Path $TraceDir | Out-Null
    $PrefixA = Join-Path $TraceDir "run-a"; $PrefixB = Join-Path $TraceDir "run-b"
    [void](Run-Logged "compatibility trace A" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixA, "-game", $Game) "bp090-094r15-trace-a.log")
    [void](Run-Logged "compatibility trace B" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixB, "-game", $Game) "bp090-094r15-trace-b.log")
    [void](Run-Logged "byte-identical trace comparison" $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", (Join-Path $Build "bp090-094r15-trace-comparison.json")) "bp090-094r15-trace-comparison.log")
  ...
  if (-not $SkipBlackPortCorpus -and $CanUseGame) {
    $CorpusRoot = Join-Path $Build "bp090-094r15-black-port-corpus"
    ...
    foreach ($Scenario in $CorpusScenarios) {
      ...
      [void](Run-Logged ("black-port corpus {0} trace A" -f $ScenarioName) $GameExe @("--compat-trace", $QuakeBase, $ScenarioMap, [string]$BlackPortCorpusFrames, $PrefixA, "-game", $Game) ("bp090-094r15-corpus-{0}-a.log" -f $ScenarioName))
      [void](Run-Logged ("black-port corpus {0} trace B" -f $ScenarioName) $GameExe @("--compat-trace", $QuakeBase, $ScenarioMap, [string]$BlackPortCorpusFrames, $PrefixB, "-game", $Game) ("bp090-094r15-corpus-{0}-b.log" -f $ScenarioName))
      $ComparisonJson = Join-Path $Build ("bp090-094r15-corpus-{0}-comparison.json" -f $ScenarioName)
      [void](Run-Logged ("black-port corpus {0} comparison" -f $ScenarioName) $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", $ComparisonJson) ("bp090-094r15-corpus-{0}-comparison.log" -f $ScenarioName))
    ...
    $CorpusSummaryPath = Join-Path $Build "bp090-094r15-black-port-corpus-summary.json"
    [ordered]@{
      schema_version = 1
      status = "PASS"
  ...
  if (-not $SkipSoak -and $CanUseGame) {
    [void](Run-Logged "5000-frame host soak" $GameExe @("--soak", $QuakeBase, $Map, [string]$SoakFrames, "-game", $Game) "bp090-094r15-host-soak.log" "result=PASS")
    $SoakPort = Get-Random -Minimum 30000 -Maximum 60000
    [void](Run-Logged "5000-frame listen-server resource soak" $GameExe @("--long-soak", "listen", $QuakeBase, $Map, [string]$ListenSoakFrames, "-game", $Game, "-port", [string]$SoakPort) "bp090-094r15-listen-soak.log" "result=PASS")
  } else {
    Add-Step "resource stability soaks" "SKIPPED" 0 "skipped or no usable Quake data"
  }

  if (-not $SkipRenderEvidence -and $CanUseGame) {
    $EvidenceDir = Join-Path $Build "bp090-094r15-render"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $EvidenceDir; New-Item -ItemType Directory -Force -Path $EvidenceDir | Out-Null
    $EA = Join-Path $EvidenceDir "a"; $EB = Join-Path $EvidenceDir "b"
    [void](Run-Logged "render evidence A" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EA, "-game", $Game) "bp090-094r15-render-a.log")
    [void](Run-Logged "render evidence B" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EB, "-game", $Game) "bp090-094r15-render-b.log")
    [void](Run-Logged "byte-identical render evidence" $PythonExe @($RenderComparator, ("{0}.tga" -f $EA), ("{0}.tga" -f $EB), "--require-exact", "--json-out", (Join-Path $Build "bp090-094r15-render-comparison.json")) "bp090-094r15-render-comparison.log")
  ...
  if ($NetworkTests) { [void](Run-Logged "Winsock UDP loopback smoke" $GameExe @("--udp-smoke", "2000") "bp090-094r15-udp-smoke.log" "result=PASS") }
  ...
  if ($SkipOriginalInterop -or $SkipOriginalVisualReference) {
    throw "final BP-090-094R15 acceptance requires original binary interop and external GLQuake visual-reference gates"
  }
  Add-Step "MiniQuake BP-090-094R15 acceptance" "PASS" 0 "all internal and external gates passed"
} catch {
  ...
  $Summary = [ordered]@{
    ...
    original_server_readiness = "protocol3_server_info_response"
    original_server_ready_timeout_ms = $OriginalServerReadyTimeoutMs
...
if (-not [string]::IsNullOrWhiteSpace($Failure)) {
  Write-Host "MiniQuake BP-090-094R15 acceptance test: FAIL" -ForegroundColor Red
  Write-Host "Run .\scripts\COLLECT_RESULTS.ps1 and upload the generated ZIP."
  exit 1
}
Write-Host "MiniQuake BP-090-094R15 acceptance test: PASS" -ForegroundColor Green
