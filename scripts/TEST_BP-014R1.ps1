...
$TranscriptPath = Join-Path $BuildDirectory ("bp014r1-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $BuildDirectory "bp014r1-test-summary.json"
$StaticReportPath = Join-Path $BuildDirectory "bp014r1-static-verification.json"
$VersionPath = Join-Path $BuildDirectory "bp014r1-version.txt"
$BuildChildLogPath = Join-Path $BuildDirectory "bp014r1-build-child.log"
$TraceALogPath = Join-Path $BuildDirectory "bp014r1-trace-a.log"
$TraceBLogPath = Join-Path $BuildDirectory "bp014r1-trace-b.log"
$SnapshotCommandLogPath = Join-Path $BuildDirectory "bp014r1-snapshot-command.log"
$TraceComparisonPath = Join-Path $BuildDirectory "bp014r1-trace-comparison.json"
$TraceComparisonLogPath = Join-Path $BuildDirectory "bp014r1-trace-comparison.log"
$ProtocolVectorReportPath = Join-Path $BuildDirectory "bp014r1-protocol15-vectors.json"
$ProtocolVectorLogPath = Join-Path $BuildDirectory "bp014r1-protocol15-vectors.log"
$ProtocolCommandReportPath = Join-Path $BuildDirectory "bp014r1-protocol15-commands.json"
$ProtocolCommandLogPath = Join-Path $BuildDirectory "bp014r1-protocol15-commands.log"
$ProtocolServerDataReportPath = Join-Path $BuildDirectory "bp014r1-protocol15-serverdata.json"
$ProtocolServerDataLogPath = Join-Path $BuildDirectory "bp014r1-protocol15-serverdata.log"
$ProtocolEventReportPath = Join-Path $BuildDirectory "bp014r1-protocol15-events.json"
$ProtocolEventLogPath = Join-Path $BuildDirectory "bp014r1-protocol15-events.log"
$ProtocolRuntimeEventReportPath = Join-Path $BuildDirectory "bp014r1-protocol15-runtime-events.json"
$ProtocolRuntimeEventLogPath = Join-Path $BuildDirectory "bp014r1-protocol15-runtime-events.log"
...
try {
  ...
  Invoke-ExternalStep `
    ...
    -Detail "manifest, packages, ABI, accepted BP-014 source/runtime-event parent and BP-014R1 beam retained-state/active-view hotfix contract"
  ...
  if (-not $SkipBuild) {
    ...
    Invoke-ExternalStep `
      -Name "build plus core, milestone, diagnostics and Protocol 15 wire/command/server-data/event/runtime-event tests" `
      -Executable $PowerShellExe `
      -Arguments $BuildArguments `
      -Detail "MiniQuake.exe plus 16 core, 24 milestone, 10 diagnostics, 15 wire, 14 command/update, 17 server-data, 22 event and 28 runtime-event fixtures" `
      -LogPath $BuildChildLogPath
  } else {
    Add-StepResult "build plus core, milestone, diagnostics and Protocol 15 wire/command/server-data/event/runtime-event tests" "SKIPPED" 0 "-SkipBuild"
  }
  ...
  if (-not $CanTrace) {
    ...
    foreach ($Artifact in @($TraceA, $TraceB, $SnapshotA, $ContextA, $SummaryAPath, $SummaryBPath)) {
      Require-File $Artifact "BP-014R1 trace artifact"
    }
    ...
    if ($ComparisonExitCode -ne 0 -or -not [bool]$Comparison.equal) {
      ...
      throw "BP-014R1 deterministic traces differ. The first differing fields are recorded in $TraceComparisonPath."
    }
  ...
  $Summary = [ordered]@{
    ...
    artifacts = [ordered]@{
      ...
      trace_directory = "bp014r1-traces"
    }
    steps = @($Steps)
  }
