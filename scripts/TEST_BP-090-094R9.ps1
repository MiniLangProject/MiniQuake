...
$BlockId = "BP-090-094"
$DeliveryRevision = "BP-090-094R9"
$DeliveryParent = "BP-090-094R8"
...
$TranscriptPath = Join-Path $Build ("bp090-094r9-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $Build "bp090-094r9-test-summary.json"
$Steps = New-Object System.Collections.Generic.List[object]
$FirewallReportPath = Join-Path $Build "bp090-094r9-temporary-firewall-rules.json"
$FirewallRulePrefix = "MiniQuake-BP090094R8-Temporary"
$FirewallDisplayGroup = "MiniQuake BP-090-094R9 temporary loopback interop"
$FirewallRuleNames = @()
...
function Write-TemporaryFirewallReport([string]$Status, [string]$ErrorText) {
  ...
  [ordered]@{
    schema_version = 1
    delivery_revision = 'BP-090-094R9'
    status = $Status
...
function Run-NetworkEvidencePair([string]$Suffix, [int]$Port) {
  $ServerOut = Join-Path $Build ("bp090-094r9-network-server-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp090-094r9-network-server-{0}.err.log" -f $Suffix)
  $ClientLog = Join-Path $Build ("bp090-094r9-network-client-{0}.log" -f $Suffix)
  $PairJson = Join-Path $Build ("bp090-094r9-network-pair-{0}.json" -f $Suffix)
...
function Run-OriginalServerMiniClientPair([string]$Suffix, [int]$Port) {
  ...
  $OriginalServerOut = Join-Path $Build ("bp090-094r9-original-server-{0}-stdout.log" -f $Suffix)
  $OriginalServerErr = Join-Path $Build ("bp090-094r9-original-server-{0}-stderr.log" -f $Suffix)
  $ProcessReportPath = Join-Path $Build ("bp090-094r9-original-server-{0}-process.json" -f $Suffix)
  ...
  try {
    ...
    while ([DateTime]::UtcNow -lt $Deadline -and -not $SignonComplete) {
      ...
      $Attempt = $Attempt + 1
      $AttemptPrefix = Join-Path $Build ("bp090-094r9-miniquake-client-original-server-{0}-attempt-{1}" -f $Suffix, $Attempt)
      $AttemptLogName = "bp090-094r9-miniquake-client-original-server-{0}-attempt-{1}.log" -f $Suffix, $Attempt
    ...
    $Normalized = Join-Path $Build ("bp090-094r9-original-server-mini-client-{0}-normalized.json" -f $Suffix)
    ...
    try {
      [ordered]@{
        ...
        readiness_evidence = 'miniquake_target_udp_provenance_and_protocol15_signon4'
        attempts = $Attempt
...
function Run-MiniServerOriginalClientPair([string]$Suffix, [int]$Port) {
  $Prefix = Join-Path $Build ("bp090-094r9-miniquake-server-original-client-{0}" -f $Suffix)
  $ReadyPath = $Prefix + '-ready.json'
  $SummaryPath = $Prefix + '-summary.json'
  $ServerOut = Join-Path $Build ("bp090-094r9-miniquake-server-original-client-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp090-094r9-miniquake-server-original-client-{0}.err.log" -f $Suffix)
  $ProcessReportPath = Join-Path $Build ("bp090-094r9-original-client-{0}-process.json" -f $Suffix)
  ...
  try {
    ...
    $Normalized = Join-Path $Build ("bp090-094r9-mini-server-original-client-{0}-normalized.json" -f $Suffix)
...
function Run-OriginalVisualCapture([string]$Demo, [int]$Frame, [string]$Suffix, [string]$ScenarioRoot) {
  ...
  $ProcessReportPath = Join-Path $Build ("bp090-094r9-original-visual-{0}-{1}-process.json" -f $Demo, $Suffix)
...
$Failure = ""
try {
  Write-Host "MiniQuake BP-090-094R9 acceptance test"
  ...
  if (-not $SkipBuild) {
    ...
    [void](Run-Logged "single cumulative build and unit-test suite" $PowerShellExe $Arguments "bp090-094r9-build-child.log" "MiniQuake BP-094 external compatibility closure tests passed: 24")
  } else {
  ...
  $Version = Run-Logged "package identity" $GameExe @("--version") "bp090-094r9-version.txt" "Compatibility release status: compat_109_release_candidate_v1"
  ...
  $OriginalStage = Join-Path $Build 'bp090-094r9-original-reference'
  $OriginalExe = ''
  $OriginalBase = ''
  if ($NeedOriginalReference) {
    ...
    $PrepareArguments = @($OriginalPrepareTool, '--quake-base', $QuakeBase, '--stage', $OriginalStage, '--json', (Join-Path $Build 'bp090-094r9-original-reference-provenance.json'))
    ...
    [void](Run-Logged 'verified original GLQuake reference' $PythonExe $PrepareArguments 'bp090-094r9-original-reference.log' 'result=PASS')
  ...
  if (-not $SkipOriginalVisualReference) {
    $VisualRoot = Join-Path $Build 'bp090-094r9-original-visual'
    ...
    foreach ($DemoName in @('demo1', 'demo2', 'demo3')) {
      ...
      for ($Offset = -2; $Offset -le 2; $Offset++) {
        ...
        ) ("bp090-094r9-{0}-frame-{1}.log" -f $DemoName, $FrameNumber) 'MiniQuake render evidence: PASS')
      ...
      [void](Run-Logged ("raw original visual comparison {0}" -f $Scenario) $PythonExe $CompareArgs ("bp090-094r9-{0}-comparison.log" -f $Scenario) 'result=PASS')
    ...
    $VisualSummary = Join-Path $Build 'bp090-094r9-original-visual-summary.json'
  ...
  if ($CanUseGame) {
    ...
    [void](Run-Logged "installed Quake data validation" $GameExe @("--validate-game", $QuakeBase, $Map, "-game", $Game) "bp090-094r9-game-validation.log" "Validation result: PASS")
    [void](Run-Logged "headless runtime validation" $GameExe @("--validate-runtime", $QuakeBase, $Map, [string]$Frames, "-game", $Game) "bp090-094r9-runtime-validation.log")
  } else {
  ...
  if (-not $SkipMissionPackEvidence -and $CanUseGame) {
    foreach ($Mission in @("rogue", "hipnotic")) {
      ...
      $MissionFlag = "-" + $Mission
      [void](Run-Logged ("mission-pack {0} game validation" -f $Mission) $GameExe @("--validate-game", $QuakeBase, "start", $MissionFlag) ("bp090-094r9-{0}-validation.log" -f $Mission) "Validation result: PASS")
      [void](Run-Logged ("mission-pack {0} headless runtime" -f $Mission) $GameExe @("-basedir", $QuakeBase, $MissionFlag, "-headless", "-nosound", "-maxframes", "128", "+map", "start") ("bp090-094r9-{0}-runtime.log" -f $Mission))
    }
  } else {
  ...
  if ($CanUseGame) {
    $CoreA = Join-Path $Build "bp090-094r9-core-assets-a.log"
    $CoreB = Join-Path $Build "bp090-094r9-core-assets-b.log"
  ...
  if (-not $SkipAudioEvidence -and $CanUseGame) {
    ...
    $A = Join-Path $Build "bp090-094r9-audio-a.log"; $B = Join-Path $Build "bp090-094r9-audio-b.log"
  ...
  if (-not $SkipArtifactEvidence -and $CanUseGame) {
    $ArtifactA = Join-Path $Build "bp090-094r9-artifacts-a.log"
    $ArtifactB = Join-Path $Build "bp090-094r9-artifacts-b.log"
  ...
  if (-not $SkipTraceValidation -and $CanUseGame) {
    $TraceDir = Join-Path $Build "bp090-094r9-traces"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $TraceDir; New-Item -ItemType Directory -Force -Path $TraceDir | Out-Null
    $PrefixA = Join-Path $TraceDir "run-a"; $PrefixB = Join-Path $TraceDir "run-b"
    [void](Run-Logged "compatibility trace A" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixA, "-game", $Game) "bp090-094r9-trace-a.log")
    [void](Run-Logged "compatibility trace B" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixB, "-game", $Game) "bp090-094r9-trace-b.log")
    [void](Run-Logged "byte-identical trace comparison" $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", (Join-Path $Build "bp090-094r9-trace-comparison.json")) "bp090-094r9-trace-comparison.log")
  ...
  if (-not $SkipBlackPortCorpus -and $CanUseGame) {
    $CorpusRoot = Join-Path $Build "bp090-094r9-black-port-corpus"
    ...
    foreach ($Scenario in $CorpusScenarios) {
      ...
      [void](Run-Logged ("black-port corpus {0} trace A" -f $ScenarioName) $GameExe @("--compat-trace", $QuakeBase, $ScenarioMap, [string]$BlackPortCorpusFrames, $PrefixA, "-game", $Game) ("bp090-094r9-corpus-{0}-a.log" -f $ScenarioName))
      [void](Run-Logged ("black-port corpus {0} trace B" -f $ScenarioName) $GameExe @("--compat-trace", $QuakeBase, $ScenarioMap, [string]$BlackPortCorpusFrames, $PrefixB, "-game", $Game) ("bp090-094r9-corpus-{0}-b.log" -f $ScenarioName))
      $ComparisonJson = Join-Path $Build ("bp090-094r9-corpus-{0}-comparison.json" -f $ScenarioName)
      [void](Run-Logged ("black-port corpus {0} comparison" -f $ScenarioName) $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", $ComparisonJson) ("bp090-094r9-corpus-{0}-comparison.log" -f $ScenarioName))
    ...
    $CorpusSummaryPath = Join-Path $Build "bp090-094r9-black-port-corpus-summary.json"
    [ordered]@{
      schema_version = 1
      status = "PASS"
  ...
  if (-not $SkipSoak -and $CanUseGame) {
    [void](Run-Logged "5000-frame host soak" $GameExe @("--soak", $QuakeBase, $Map, [string]$SoakFrames, "-game", $Game) "bp090-094r9-host-soak.log" "result=PASS")
    $SoakPort = Get-Random -Minimum 30000 -Maximum 60000
    [void](Run-Logged "5000-frame listen-server resource soak" $GameExe @("--long-soak", "listen", $QuakeBase, $Map, [string]$ListenSoakFrames, "-game", $Game, "-port", [string]$SoakPort) "bp090-094r9-listen-soak.log" "result=PASS")
  } else {
    Add-Step "resource stability soaks" "SKIPPED" 0 "skipped or no usable Quake data"
  }

  if (-not $SkipRenderEvidence -and $CanUseGame) {
    $EvidenceDir = Join-Path $Build "bp090-094r9-render"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $EvidenceDir; New-Item -ItemType Directory -Force -Path $EvidenceDir | Out-Null
    $EA = Join-Path $EvidenceDir "a"; $EB = Join-Path $EvidenceDir "b"
    [void](Run-Logged "render evidence A" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EA, "-game", $Game) "bp090-094r9-render-a.log")
    [void](Run-Logged "render evidence B" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EB, "-game", $Game) "bp090-094r9-render-b.log")
    [void](Run-Logged "byte-identical render evidence" $PythonExe @($RenderComparator, ("{0}.tga" -f $EA), ("{0}.tga" -f $EB), "--require-exact", "--json-out", (Join-Path $Build "bp090-094r9-render-comparison.json")) "bp090-094r9-render-comparison.log")
  ...
  if ($NetworkTests) { [void](Run-Logged "Winsock UDP loopback smoke" $GameExe @("--udp-smoke", "2000") "bp090-094r9-udp-smoke.log" "result=PASS") }
  ...
  if ($SkipOriginalInterop -or $SkipOriginalVisualReference) {
    throw "final BP-090-094R9 acceptance requires original binary interop and external GLQuake visual-reference gates"
  }
  Add-Step "MiniQuake BP-090-094R9 acceptance" "PASS" 0 "all internal and external gates passed"
} catch {
...
if (-not [string]::IsNullOrWhiteSpace($Failure)) {
  Write-Host "MiniQuake BP-090-094R9 acceptance test: FAIL" -ForegroundColor Red
  Write-Host "Run .\scripts\COLLECT_RESULTS.ps1 and upload the generated ZIP."
  exit 1
}
Write-Host "MiniQuake BP-090-094R9 acceptance test: PASS" -ForegroundColor Green
