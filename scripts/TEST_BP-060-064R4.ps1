...
$PackageId = "BP-064"
$ParentPackageId = "BP-063"
$BlockId = "BP-060-064"
$DeliveryRevision = "BP-060-064R4"
$BlockParentPackageId = "BP-055-059R2"
...
$TranscriptPath = Join-Path $Build ("bp060-064r4-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $Build "bp060-064r4-test-summary.json"
...
function Run-NetworkEvidencePair([string]$Suffix, [int]$Port) {
  $ServerOut = Join-Path $Build ("bp060-064r4-network-server-{0}.log" -f $Suffix)
  $ServerErr = Join-Path $Build ("bp060-064r4-network-server-{0}.err.log" -f $Suffix)
  $ClientLog = Join-Path $Build ("bp060-064r4-network-client-{0}.log" -f $Suffix)
  Remove-Item -Force -ErrorAction SilentlyContinue $ServerOut, $ServerErr, $ClientLog
  Write-Host ("[MiniQuake] starting network evidence server {0} on UDP port {1}" -f $Suffix, $Port) -ForegroundColor Cyan
  $Server = Start-Process -FilePath $EvidenceExe -ArgumentList @("server", [string]$Port) -PassThru -NoNewWindow -RedirectStandardOutput $ServerOut -RedirectStandardError $ServerErr
  ...
  if (-not $Server.WaitForExit(7000)) { Stop-Process -Id $Server.Id -Force -ErrorAction SilentlyContinue; throw "network evidence server $Suffix timed out" }
  $ServerCode = $Server.ExitCode
  $ServerText = ""
  if (Test-Path $ServerOut) { $ServerText = Get-Content -Raw -LiteralPath $ServerOut }
  if (Test-Path $ServerErr) { $ServerText += Get-Content -Raw -LiteralPath $ServerErr }
  if (-not [string]::IsNullOrWhiteSpace($ServerText)) {
    Write-Host ("[MiniQuake] network evidence server {0} output" -f $Suffix) -ForegroundColor Cyan
    foreach ($ServerLine in ($ServerText -split "`r?`n")) {
      if (-not [string]::IsNullOrEmpty($ServerLine)) { Write-Host $ServerLine }
    }
  }
  if ($ServerCode -ne 0 -or $ClientCode -ne 0) { throw "network evidence pair $Suffix failed: server=$ServerCode client=$ClientCode" }
  if ($ServerText -notmatch 'network platform evidence server: PASS') { throw "network evidence server $Suffix did not report PASS" }
  if ($ClientText -notmatch 'network platform evidence: PASS') { throw "network evidence client $Suffix did not report PASS" }
  return $ClientLog
}
...
$Failure = ""
try {
  Write-Host "MiniQuake BP-060-064R4 acceptance test"
  ...
  if (-not $SkipBuild) {
    ...
    [void](Run-Logged "single cumulative build and unit-test suite" $PowerShellExe $Arguments "bp060-064r4-build-child.log" "MiniQuake BP-064 network/platform closure tests passed: 24")
  } else {
    Add-Step "single cumulative build and unit-test suite" "SKIPPED" 0 "-SkipBuild"
  }

  foreach ($Path in @($GameExe, $EvidenceExe)) { if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "required executable missing: $Path" } }

  $Version = Run-Logged "package identity" $GameExe @("--version") "bp060-064r4-version.txt" "Network/platform status: network_platform_109_frozen_v1"
  if ($Version -notmatch 'Package: BP-064' -or $Version -notmatch 'Block: BP-060-064') { throw "compiled package identity mismatch" }
  ...
  if ($CanUseGame) {
    ...
    [void](Run-Logged "installed Quake data validation" $GameExe @("--validate-game", $QuakeBase, $Map, "-game", $Game) "bp060-064r4-game-validation.log" "Validation result: PASS")
    [void](Run-Logged "headless runtime validation" $GameExe @("--validate-runtime", $QuakeBase, $Map, [string]$Frames, "-game", $Game) "bp060-064r4-runtime-validation.log")
  } else {
  ...
  if (-not $SkipAudioEvidence -and $CanUseGame) {
    ...
    $A = Join-Path $Build "bp060-064r4-audio-a.log"; $B = Join-Path $Build "bp060-064r4-audio-b.log"
  ...
  if (-not $SkipTraceValidation -and $CanUseGame) {
    $TraceDir = Join-Path $Build "bp060-064r4-traces"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $TraceDir; New-Item -ItemType Directory -Force -Path $TraceDir | Out-Null
    $PrefixA = Join-Path $TraceDir "run-a"; $PrefixB = Join-Path $TraceDir "run-b"
    [void](Run-Logged "compatibility trace A" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixA, "-game", $Game) "bp060-064r4-trace-a.log")
    [void](Run-Logged "compatibility trace B" $GameExe @("--compat-trace", $QuakeBase, $Map, [string]$TraceFrames, $PrefixB, "-game", $Game) "bp060-064r4-trace-b.log")
    [void](Run-Logged "byte-identical trace comparison" $PythonExe @($TraceComparator, ("{0}.mqtrace" -f $PrefixA), ("{0}.mqtrace" -f $PrefixB), "--json-output", (Join-Path $Build "bp060-064r4-trace-comparison.json")) "bp060-064r4-trace-comparison.log")
  ...
  if (-not $SkipRenderEvidence -and $CanUseGame) {
    $EvidenceDir = Join-Path $Build "bp060-064r4-render"; Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $EvidenceDir; New-Item -ItemType Directory -Force -Path $EvidenceDir | Out-Null
    $EA = Join-Path $EvidenceDir "a"; $EB = Join-Path $EvidenceDir "b"
    [void](Run-Logged "render evidence A" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EA, "-game", $Game) "bp060-064r4-render-a.log")
    [void](Run-Logged "render evidence B" $GameExe @("--render-evidence", $QuakeBase, $Map, [string]$RenderEvidenceFrame, $EB, "-game", $Game) "bp060-064r4-render-b.log")
    [void](Run-Logged "byte-identical render evidence" $PythonExe @($RenderComparator, ("{0}.tga" -f $EA), ("{0}.tga" -f $EB), "--require-exact", "--json-output", (Join-Path $Build "bp060-064r4-render-comparison.json")) "bp060-064r4-render-comparison.log")
  ...
  if ($NetworkTests) { [void](Run-Logged "Winsock UDP loopback smoke" $GameExe @("--udp-smoke", "2000") "bp060-064r4-udp-smoke.log" "result=PASS") }
  ...
  Add-Step "MiniQuake BP-060-064R4 acceptance" "PASS" 0 "all required gates passed"
} catch {
...
if (-not [string]::IsNullOrWhiteSpace($Failure)) {
  Write-Host "MiniQuake BP-060-064R4 acceptance test: FAIL" -ForegroundColor Red
  Write-Host "Run .\scripts\COLLECT_RESULTS.ps1 and upload the generated ZIP."
  exit 1
}
Write-Host "MiniQuake BP-060-064R4 acceptance test: PASS" -ForegroundColor Green
