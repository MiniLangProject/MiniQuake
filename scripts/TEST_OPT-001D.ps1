...
$TargetRenderMedianMs = 16.67
$TargetRenderP95Ms = 20.0
$TargetRenderP99Ms = 25.0
$SummaryPath = Join-Path $Build "opt001d-test-summary.json"
...
function Show-LogTail([string]$LogPath, [int]$Lines = 80) {
  if (-not (Test-Path -LiteralPath $LogPath -PathType Leaf)) {
    Write-Host ("[MiniQuake/OPT-001D] log was not created: {0}" -f $LogPath) -ForegroundColor Red
    return
  }
  Write-Host ("[MiniQuake/OPT-001D] last {0} log lines:" -f $Lines) -ForegroundColor Yellow
...
function Invoke-ExternalProcessLive(
  ...
  Write-Host ("[MiniQuake/OPT-001D] starting {0}" -f $Name) -ForegroundColor Cyan
  ...
  if (Test-Path -LiteralPath $StatusPath -PathType Leaf) {
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$Status.error)) {
      Write-Host "[MiniQuake/OPT-001D] live-runner diagnostic:" -ForegroundColor Red
      Write-Host ([string]$Status.error) -ForegroundColor DarkRed
    }
  }
  elseif ($RunnerExitCode -eq 0) {
    $Code = 125
    Write-Host "[MiniQuake/OPT-001D] live runner returned 0 but did not create its status JSON." -ForegroundColor Red
  }
  if ($Code -eq 0) {
...
function Install-LoopbackFirewallRules {
  if ($SkipTemporaryFirewallRules) {
    Write-Host "[MiniQuake/OPT-001D] temporary loopback firewall rules explicitly skipped" -ForegroundColor Yellow
    return
  }
  if (-not (Test-IsAdministrator)) {
    throw "OPT-001D listen-server measurement requires elevated PowerShell for unattended temporary loopback firewall rules. Start PowerShell as Administrator or pass -SkipTemporaryFirewallRules."
  }
  ...
  Write-Host "[MiniQuake/OPT-001D] temporary loopback firewall rules installed: 2" -ForegroundColor Green
}
...
function Remove-LoopbackFirewallRules {
  ...
  if ($script:FirewallRules.Count -gt 0) {
    Write-Host "[MiniQuake/OPT-001D] temporary loopback firewall rules removed" -ForegroundColor Green
  }
}

function Write-Summary([string]$Overall, [string]$HandleClassification, [string]$PerformanceClassification) {
  [ordered]@{
    schema = "MiniQuakeOPT001DTestSummary/1"
...
function Test-OPT001D60FpsGate {
  param([string]$BuildRoot)
  $Scenarios = @('e1m1','e1m2')
  $Failures = @()
  foreach ($Scenario in $Scenarios) {
    $Path = Join-Path $BuildRoot ("opt001d-{0}-render-summary.json" -f $Scenario)
    if (-not (Test-Path -LiteralPath $Path)) {
      $Failures += "missing render summary: $Path"
      continue
    }
    $Data = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
    $Median = [double]$Data.median_ms
    $P95 = [double]$Data.p95_ms
    $P99 = [double]$Data.p99_ms
    $Fps = if ($Median -gt 0) { 1000.0 / $Median } else { 0.0 }
    Write-Host ("[MiniQuake/OPT-001D] {0}: median={1}ms ({2:N1} FPS) p95={3}ms p99={4}ms" -f $Scenario,$Median,$Fps,$P95,$P99)
    if ($Median -gt $TargetRenderMedianMs) { $Failures += "$Scenario median $Median ms exceeds $TargetRenderMedianMs ms" }
    if ($P95 -gt $TargetRenderP95Ms) { $Failures += "$Scenario p95 $P95 ms exceeds $TargetRenderP95Ms ms" }
    if ($P99 -gt $TargetRenderP99Ms) { $Failures += "$Scenario p99 $P99 ms exceeds $TargetRenderP99Ms ms" }
  }
  $GatePath = Join-Path $BuildRoot 'opt001d-60fps-gate.json'
  [pscustomobject]@{
    schema = 'MiniQuakeOPT001D60FpsGate/1'
    target_median_ms = $TargetRenderMedianMs
    target_p95_ms = $TargetRenderP95Ms
    target_p99_ms = $TargetRenderP99Ms
    pass = ($Failures.Count -eq 0)
    failures = $Failures
  } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $GatePath -Encoding UTF8
  if ($Failures.Count -gt 0) {
    foreach ($Failure in $Failures) { Write-Host "[FAIL] 60 FPS gate: $Failure" -ForegroundColor Red }
    return 1
  }
  Write-Host '[PASS] OPT-001D 60 FPS render gate' -ForegroundColor Green
  return 0
}
...
Write-Host "MiniQuake OPT-001D acceptance test"
...
try {
  $SourceVerifyCode = Invoke-Live "OPT-001D source/performance verification" $PythonExe @(
    (Join-Path $Root "tools\check_opt001d.py"), "--root", $Root,
    "--json", (Join-Path $Build "opt001d-source-verification.json")
  ) "opt001d-source-verification.log"
  $SourceVerifyCode = [int]$SourceVerifyCode
  if ($SourceVerifyCode -ne 0) {
    Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
    Write-Host "MiniQuake OPT-001D acceptance test: FAIL" -ForegroundColor Red
    exit 2
  }

  $PackageVerifyCode = Invoke-Live "OPT-001D package verification" $PythonExe @(
    (Join-Path $Root "tools\verify.py"), "--root", $Root,
    "--json", (Join-Path $Build "opt001d-package-verification.json")
  ) "opt001d-package-verification.log"
  ...
  if (-not $SkipBuild) {
    $BuildCode = Invoke-LiveBuild `
      "OPT-001D game and contract build" `
    ...
    Add-Step "OPT-001D game and contract build" "SKIPPED" 0 "-SkipBuild"
  }

  if (-not $SkipBuild -and $BuildCode -ne 0) {
    Add-Step "OPT-001D allocation contract tests" "SKIPPED" 0 "dependent tests skipped because build failed"
    Add-Step "OPT-001D inline/array hotpath tests" "SKIPPED" 0 "dependent tests skipped because build failed"
    Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  ...
  foreach ($RequiredExe in @($GameExe, $ContractExe, $HotpathExe)) {
    if (-not (Test-Path -LiteralPath $RequiredExe -PathType Leaf)) {
      Add-Step "OPT-001D required executable" "FAIL" 1 $RequiredExe
      Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  ...
  foreach ($Map in @("e1m1", "e1m2")) {
    ...
    foreach ($Mode in @("headless", "render")) {
      ...
      ) ("opt001d-{0}-{1}-baseline.log" -f $Map, $Mode))
    }
  }

  $VisiblePrefix = Join-Path $Build "opt001d-e1m2-visible"
  [void](Invoke-Live "OPT-001D e1m2 visible validation" $GameExe @(
    "--render-evidence", $QuakeBase, "e1m2", [string]$E1M2VisibleFrames, $VisiblePrefix, "-game", $Game
  ) "opt001d-e1m2-visible.log")

  [void](Invoke-Live "OPT-001D e1m2 headless validation" $GameExe @(
    "--runtime-smoke", $QuakeBase, "e1m2", [string]$E1M2HeadlessFrames, "-game", $Game
  ) "opt001d-e1m2-headless.log")

  $TransitionPrefix = Join-Path $Build "opt001d-transition"
  [void](Invoke-Live "OPT-001D e1m1-e1m2-e1m1 transition" $GameExe @(
    "--opt001b-transition", $QuakeBase, [string]$TransitionFrames, $TransitionPrefix, "-game", $Game
  ) "opt001d-transition.log")

  $PlateauPrefix = Join-Path $Build "opt001d-listen-handle-plateau"
  ...
  ) "opt001d-handle-plateau.log" -AllowFailure)

  [void](Invoke-Live "OPT-001D aggregate analysis" $PythonExe @(
    (Join-Path $Root "tools\analyze_opt001a.py"), "--build", $Build,
    "--prefix", "opt001d", "--next-revision", "OPT-001D",
    "--json", (Join-Path $Build "opt001d-baseline-summary.json"),
    "--markdown", (Join-Path $Build "opt001d-baseline-summary.md")
  ) "opt001d-aggregate-analysis.log" -AllowFailure)

  $PerformanceJson = Join-Path $Build "opt001d-performance-comparison.json"
  [void](Invoke-Live "OPT-001D performance comparison" $PythonExe @(
    ...
    "--markdown", (Join-Path $Build "opt001d-performance-comparison.md")
  ) "opt001d-performance-comparison.log")

  [void](Invoke-Live "OPT-001D incremental performance comparison" $PythonExe @(
    (Join-Path $Root "tools\compare_opt001cr3_performance.py"),
    "--baseline", (Join-Path $Root "audit\opt001cr2_accepted_baseline.json"),
    "--build", $Build,
    "--prefix", "opt001d",
    "--json", (Join-Path $Build "opt001d-incremental-performance.json"),
    "--markdown", (Join-Path $Build "opt001d-incremental-performance.md")
  ) "opt001d-incremental-performance.log" -AllowFailure)

  [void](Invoke-Live "OPT-001D audio cost analysis" $PythonExe @(
    (Join-Path $Root "tools\analyze_opt001cr3r8_audio.py"),
    "--build", $Build,
    "--prefix", "opt001d",
    "--json", (Join-Path $Build "opt001d-audio-analysis.json")
  ) "opt001d-audio-analysis.log" -AllowFailure)
  ...
  $Local60FpsCode = [int](Test-OPT001D60FpsGate -BuildRoot $Build)
  if ($Local60FpsCode -eq 0) {
    Add-Step "OPT-001D local 60 FPS gate" "PASS" 0 "opt001d-60fps-gate.json"
  } else {
    Add-Step "OPT-001D local 60 FPS gate" "FAIL" $Local60FpsCode "opt001d-60fps-gate.json"
  }

  [void](Invoke-Live "OPT-001D hard 60 FPS gate" $PythonExe @(
    (Join-Path $Root "tools\check_opt001d_60fps.py"),
    "--build", $Build,
    "--prefix", "opt001d",
    "--json", (Join-Path $Build "opt001d-60fps-check.json")
  ) "opt001d-60fps-check.log")
  ...
  Write-Host ""
  Write-Host "MiniQuake OPT-001D summary"
  ...
  if ($Overall -eq "PASS") {
    Write-Host "MiniQuake OPT-001D acceptance test: PASS" -ForegroundColor Green
    exit 0
  }
  Write-Host ("MiniQuake OPT-001D acceptance test: {0}" -f $Overall) -ForegroundColor Red
  exit 2
}
catch {
  ...
  Add-Step "OPT-001D harness" "FAIL" 1 $HarnessDetail
  Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  Write-Host "ERROR: OPT-001D harness failure" -ForegroundColor Red
