...
$DeliveryRevision = "OPT-001CR3"
$DeliveryParent = "OPT-001CR2"
$SummaryPath = Join-Path $Build "opt001cr3-test-summary.json"
...
function Invoke-LivePowerShellScript(
  ...
  Write-Host ("[MiniQuake/OPT-001CR3] starting {0}" -f $Name) -ForegroundColor Cyan
  Write-Host ("  script={0}" -f $ScriptPath)
  Write-Host ("  arguments={0}" -f ($Arguments -join " "))
  Write-Host "  output_mode=attached_console_live"
  ...
  # The nested build PowerShell must inherit the current console directly.
  # Piping powershell.exe in the parent can cause Windows PowerShell 5.1 to
  # buffer its host/information stream until the child exits.  The child owns
  # the Tee-Object pipeline instead, so every line is shown immediately and
  # written to the aggregate build log at the same time.
  $ChildCommand = @"
`$ErrorActionPreference = 'Stop'
`$InformationPreference = 'Continue'
`$BuildScript = $ScriptLiteral
`$BuildArguments = $ArgumentExpression
& `$BuildScript @BuildArguments *>&1 | Tee-Object -FilePath $LogLiteral
"@
  $EncodedCommand = [Convert]::ToBase64String(
    [Text.Encoding]::Unicode.GetBytes($ChildCommand)
  )

  # Deliberately no parent-side pipeline or redirection here. The child is
  # attached to this console, which avoids the buffering regression.
...
function Install-LoopbackFirewallRules {
  if ($SkipTemporaryFirewallRules) {
    Write-Host "[MiniQuake/OPT-001CR3] temporary loopback firewall rules explicitly skipped" -ForegroundColor Yellow
    return
  }
  if (-not (Test-IsAdministrator)) {
    throw "OPT-001CR3 listen-server measurement requires elevated PowerShell for unattended temporary loopback firewall rules. Start PowerShell as Administrator or pass -SkipTemporaryFirewallRules."
  }
  ...
  Write-Host "[MiniQuake/OPT-001CR3] temporary loopback firewall rules installed: 2" -ForegroundColor Green
}
...
function Remove-LoopbackFirewallRules {
  ...
  if ($script:FirewallRules.Count -gt 0) {
    Write-Host "[MiniQuake/OPT-001CR3] temporary loopback firewall rules removed" -ForegroundColor Green
  }
}
...
Write-Host "MiniQuake OPT-001CR3 acceptance test"
...
try {
  [void](Invoke-Live "OPT-001CR3 package verification" $PythonExe @(
    (Join-Path $Root "tools\verify.py"), "--root", $Root,
    "--json", (Join-Path $Build "opt001cr3-package-verification.json")
  ) "opt001cr3-package-verification.log")
  ...
  if (-not $SkipBuild) {
    ...
    [void](Invoke-LivePowerShellScript `
      "OPT-001CR3 game and contract build" `
    ...
    Add-Step "OPT-001CR3 game and contract build" "SKIPPED" 0 "-SkipBuild"
  }

  [void](Invoke-Live "OPT-001CR3 allocation contract tests" $ContractExe @() "opt001cr3-allocation-tests.log")
  [void](Invoke-Live "OPT-001CR3 inline/array hotpath tests" $HotpathExe @() "opt001cr3-hotpath-tests.log")
  Install-LoopbackFirewallRules

  foreach ($Map in @("e1m1", "e1m2")) {
    $MapPrefix = Join-Path $Build ("opt001cr3-{0}" -f $Map)
    [void](Invoke-Live "$Map BSP parse" $GameExe @(
      "--opt001a-map-parse", $QuakeBase, $Map, $MapPrefix, "-game", $Game
    ) ("opt001cr3-{0}-map-parse.log" -f $Map))
    [void](Invoke-Live "$Map runtime smoke" $GameExe @(
      "--runtime-smoke", $QuakeBase, $Map, [string]$MatrixFrames, "-game", $Game
    ) ("opt001cr3-{0}-runtime-smoke.log" -f $Map))
    [void](Invoke-Live "$Map render smoke" $GameExe @(
      "--render-smoke", $QuakeBase, $Map, [string]$MatrixFrames, "-game", $Game
    ) ("opt001cr3-{0}-render-smoke.log" -f $Map))

    $TraceA = Join-Path $Build ("opt001cr3-{0}-trace-a" -f $Map)
    $TraceB = Join-Path $Build ("opt001cr3-{0}-trace-b" -f $Map)
    [void](Invoke-Live "$Map trace A" $GameExe @(
      "--compat-trace", $QuakeBase, $Map, [string]$MatrixFrames, $TraceA, "-game", $Game
    ) ("opt001cr3-{0}-trace-a.log" -f $Map))
    [void](Invoke-Live "$Map trace B" $GameExe @(
      "--compat-trace", $QuakeBase, $Map, [string]$MatrixFrames, $TraceB, "-game", $Game
    ) ("opt001cr3-{0}-trace-b.log" -f $Map))
    [void](Invoke-Live "$Map trace comparison" $PythonExe @(
      (Join-Path $Root "tools\compare_traces.py"), ($TraceA + ".mqtrace"), ($TraceB + ".mqtrace"),
      "--json-output", (Join-Path $Build ("opt001cr3-{0}-trace-comparison.json" -f $Map))
    ) ("opt001cr3-{0}-trace-comparison.log" -f $Map))

    foreach ($Mode in @("headless", "render")) {
      $Prefix = Join-Path $Build ("opt001cr3-{0}-{1}" -f $Map, $Mode)
      ...
      ) ("opt001cr3-{0}-{1}-baseline.log" -f $Map, $Mode))
    }
  }

  $VisiblePrefix = Join-Path $Build "opt001cr3-e1m2-visible"
  [void](Invoke-Live "OPT-001CR3 e1m2 visible validation" $GameExe @(
    "--render-evidence", $QuakeBase, "e1m2", [string]$E1M2VisibleFrames, $VisiblePrefix, "-game", $Game
  ) "opt001cr3-e1m2-visible.log")

  [void](Invoke-Live "OPT-001CR3 e1m2 headless validation" $GameExe @(
    "--runtime-smoke", $QuakeBase, "e1m2", [string]$E1M2HeadlessFrames, "-game", $Game
  ) "opt001cr3-e1m2-headless.log")

  $TransitionPrefix = Join-Path $Build "opt001cr3-transition"
  [void](Invoke-Live "OPT-001CR3 e1m1-e1m2-e1m1 transition" $GameExe @(
    "--opt001b-transition", $QuakeBase, [string]$TransitionFrames, $TransitionPrefix, "-game", $Game
  ) "opt001cr3-transition.log")

  $PlateauPrefix = Join-Path $Build "opt001cr3-listen-handle-plateau"
  ...
  ) "opt001cr3-handle-plateau.log" -AllowFailure)

  [void](Invoke-Live "OPT-001CR3 aggregate analysis" $PythonExe @(
    (Join-Path $Root "tools\analyze_opt001a.py"), "--build", $Build,
    "--prefix", "opt001cr3", "--next-revision", "OPT-001D",
    "--json", (Join-Path $Build "opt001cr3-baseline-summary.json"),
    "--markdown", (Join-Path $Build "opt001cr3-baseline-summary.md")
  ) "opt001cr3-aggregate-analysis.log" -AllowFailure)

  $PerformanceJson = Join-Path $Build "opt001cr3-performance-comparison.json"
  [void](Invoke-Live "OPT-001CR3 performance comparison" $PythonExe @(
    ...
    "--markdown", (Join-Path $Build "opt001cr3-performance-comparison.md")
  ) "opt001cr3-performance-comparison.log")

  [void](Invoke-Live "OPT-001CR3 incremental performance comparison" $PythonExe @(
    (Join-Path $Root "tools\compare_opt001cr3_performance.py"),
    "--baseline", (Join-Path $Root "audit\opt001cr2_accepted_baseline.json"),
    "--build", $Build,
    "--prefix", "opt001cr3",
    "--json", (Join-Path $Build "opt001cr3-incremental-performance.json"),
    "--markdown", (Join-Path $Build "opt001cr3-incremental-performance.md")
  ) "opt001cr3-incremental-performance.log")
  ...
  Write-Host ""
  Write-Host "MiniQuake OPT-001CR3 summary"
  ...
  if ($Overall -eq "PASS") {
    Write-Host "MiniQuake OPT-001CR3 acceptance test: PASS" -ForegroundColor Green
    exit 0
  }
  Write-Host ("MiniQuake OPT-001CR3 acceptance test: {0}" -f $Overall) -ForegroundColor Red
  exit 2
}
catch {
  Add-Step "OPT-001CR3 harness" "FAIL" 1 $_.Exception.Message
  Write-Summary "FAIL" "UNKNOWN" "UNKNOWN"
  Write-Host ("ERROR: " + $_.Exception.Message) -ForegroundColor Red
  Write-Host "MiniQuake OPT-001CR3 acceptance test: FAIL" -ForegroundColor Red
  exit 1
}
finally {
