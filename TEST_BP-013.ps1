...
$TranscriptPath = Join-Path $BuildDirectory ("bp013-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $BuildDirectory "bp013-test-summary.json"
$StaticReportPath = Join-Path $BuildDirectory "bp013-static-verification.json"
$VersionPath = Join-Path $BuildDirectory "bp013-version.txt"
$BuildChildLogPath = Join-Path $BuildDirectory "bp013-build-child.log"
$TraceALogPath = Join-Path $BuildDirectory "bp013-trace-a.log"
$TraceBLogPath = Join-Path $BuildDirectory "bp013-trace-b.log"
$SnapshotCommandLogPath = Join-Path $BuildDirectory "bp013-snapshot-command.log"
$TraceComparisonPath = Join-Path $BuildDirectory "bp013-trace-comparison.json"
$TraceComparisonLogPath = Join-Path $BuildDirectory "bp013-trace-comparison.log"
$ProtocolVectorReportPath = Join-Path $BuildDirectory "bp013-protocol15-vectors.json"
$ProtocolVectorLogPath = Join-Path $BuildDirectory "bp013-protocol15-vectors.log"
$ProtocolCommandReportPath = Join-Path $BuildDirectory "bp013-protocol15-commands.json"
$ProtocolCommandLogPath = Join-Path $BuildDirectory "bp013-protocol15-commands.log"
$ProtocolServerDataReportPath = Join-Path $BuildDirectory "bp013-protocol15-serverdata.json"
$ProtocolServerDataLogPath = Join-Path $BuildDirectory "bp013-protocol15-serverdata.log"
$ProtocolEventReportPath = Join-Path $BuildDirectory "bp013-protocol15-events.json"
$ProtocolEventLogPath = Join-Path $BuildDirectory "bp013-protocol15-events.log"
...
try {
  ...
  Invoke-ExternalStep `
    ...
    -Detail "manifest, packages, ABI, accepted BP-012R1 runtime baseline and BP-013 event/scoreboard/drop contracts"
  ...
  if (-not $SkipBuild) {
    ...
    Invoke-ExternalStep `
      -Name "build plus core, milestone, diagnostics and Protocol 15 wire/command/server-data/event tests" `
      -Executable $PowerShellExe `
      -Arguments $BuildArguments `
      -Detail "MiniQuake.exe plus 16 core, 24 milestone, 10 diagnostics, 15 wire, 14 command/update, 17 server-data and 22 event fixtures" `
      -LogPath $BuildChildLogPath
  } else {
    Add-StepResult "build plus core, milestone, diagnostics and Protocol 15 wire/command/server-data/event tests" "SKIPPED" 0 "-SkipBuild"
  }
  ...
  if (-not $CanTrace) {
    ...
    foreach ($Artifact in @($TraceA, $TraceB, $SnapshotA, $ContextA, $SummaryAPath, $SummaryBPath)) {
      Require-File $Artifact "BP-013 trace artifact"
    }
    ...
    if ($ComparisonExitCode -ne 0 -or -not [bool]$Comparison.equal) {
      ...
      throw "BP-013 deterministic traces differ. The first differing fields are recorded in $TraceComparisonPath."
    }
