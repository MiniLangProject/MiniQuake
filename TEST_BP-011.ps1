...
$TranscriptPath = Join-Path $BuildDirectory ("bp011-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $BuildDirectory "bp011-test-summary.json"
$StaticReportPath = Join-Path $BuildDirectory "bp011-static-verification.json"
$VersionPath = Join-Path $BuildDirectory "bp011-version.txt"
$BuildChildLogPath = Join-Path $BuildDirectory "bp011-build-child.log"
$TraceALogPath = Join-Path $BuildDirectory "bp011-trace-a.log"
$TraceBLogPath = Join-Path $BuildDirectory "bp011-trace-b.log"
$SnapshotCommandLogPath = Join-Path $BuildDirectory "bp011-snapshot-command.log"
$TraceComparisonPath = Join-Path $BuildDirectory "bp011-trace-comparison.json"
$TraceComparisonLogPath = Join-Path $BuildDirectory "bp011-trace-comparison.log"
$ProtocolVectorReportPath = Join-Path $BuildDirectory "bp011-protocol15-vectors.json"
$ProtocolVectorLogPath = Join-Path $BuildDirectory "bp011-protocol15-vectors.log"
$ProtocolCommandReportPath = Join-Path $BuildDirectory "bp011-protocol15-commands.json"
$ProtocolCommandLogPath = Join-Path $BuildDirectory "bp011-protocol15-commands.log"
...
try {
  ...
  Invoke-ExternalStep `
    ...
    -Detail "manifest, packages, ABI, accepted BP-010R1 regressions and BP-011 Protocol 15 signon/command/update contracts"
  ...
  if (-not $SkipBuild) {
    ...
    Invoke-ExternalStep `
      -Name "build plus core, milestone, diagnostics and Protocol 15 regression/command tests" `
      -Executable $PowerShellExe `
      -Arguments $BuildArguments `
      -Detail "MiniQuake.exe plus 16 core, 24 milestone, 10 diagnostics, 15 wire and 14 command/update fixtures" `
      -LogPath $BuildChildLogPath
  } else {
    Add-StepResult "build plus core, milestone, diagnostics and Protocol 15 regression/command tests" "SKIPPED" 0 "-SkipBuild"
  }
  ...
  if (-not $CanTrace) {
    ...
    foreach ($Artifact in @($TraceA, $TraceB, $SnapshotA, $ContextA, $SummaryAPath, $SummaryBPath)) {
      Require-File $Artifact "BP-011 trace artifact"
    }
    ...
    if ($ComparisonExitCode -ne 0 -or -not [bool]$Comparison.equal) {
      ...
      throw "BP-011 deterministic traces differ. The first differing fields are recorded in $TraceComparisonPath."
    }
