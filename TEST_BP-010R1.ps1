...
$TranscriptPath = Join-Path $BuildDirectory ("bp010r1-test-{0}.log" -f $Timestamp)
$SummaryPath = Join-Path $BuildDirectory "bp010r1-test-summary.json"
$StaticReportPath = Join-Path $BuildDirectory "bp010r1-static-verification.json"
$VersionPath = Join-Path $BuildDirectory "bp010r1-version.txt"
$BuildChildLogPath = Join-Path $BuildDirectory "bp010r1-build-child.log"
$TraceALogPath = Join-Path $BuildDirectory "bp010r1-trace-a.log"
$TraceBLogPath = Join-Path $BuildDirectory "bp010r1-trace-b.log"
$SnapshotCommandLogPath = Join-Path $BuildDirectory "bp010r1-snapshot-command.log"
$TraceComparisonPath = Join-Path $BuildDirectory "bp010r1-trace-comparison.json"
$TraceComparisonLogPath = Join-Path $BuildDirectory "bp010r1-trace-comparison.log"
$ProtocolVectorReportPath = Join-Path $BuildDirectory "bp010r1-protocol15-vectors.json"
$ProtocolVectorLogPath = Join-Path $BuildDirectory "bp010r1-protocol15-vectors.log"
...
try {
  ...
  Invoke-ExternalStep `
    ...
    -Detail "manifest, packages, ABI, BP-001R3 diagnostics regression, BP-010 Protocol 15 contract and BP-010R1 lexical-scope repair"
  ...
  Invoke-ExternalStep `
    ...
    -Detail "13 C-derived byte streams, 18 constants and MiniLang wire markers" `
  ...
  if (-not $SkipBuild) {
    ...
    Invoke-ExternalStep `
      -Name "build plus core, milestone, diagnostics and Protocol 15 tests" `
      -Executable $PowerShellExe `
      -Arguments $BuildArguments `
      -Detail "MiniQuake.exe plus 16 core, 24 milestone, 10 diagnostics and 15 Protocol 15 wire fixtures" `
      -LogPath $BuildChildLogPath
  } else {
    Add-StepResult "build plus core, milestone, diagnostics and Protocol 15 tests" "SKIPPED" 0 "-SkipBuild"
  }
  ...
  if (-not $CanTrace) {
    ...
    foreach ($Artifact in @($TraceA, $TraceB, $SnapshotA, $ContextA, $SummaryAPath, $SummaryBPath)) {
      Require-File $Artifact "BP-010R1 trace artifact"
    }
    ...
    if ($ComparisonExitCode -ne 0 -or -not [bool]$Comparison.equal) {
      ...
      throw "BP-010R1 deterministic traces differ. The first differing fields are recorded in $TraceComparisonPath."
    }
  ...
  $Summary = [ordered]@{
    ...
    artifacts = [ordered]@{
      ...
      trace_directory = "bp010r1-traces"
    }
    steps = @($Steps)
  }
